...
def remove_and_interpolate_bad_bands_chunked(wavelengths, band_data, bad_ranges=None, chunk_size=500, n_points=10):
    """
    Remove bad bands and interpolate their values using a chunked approach to save memory.
    Uses n_points before and after each bad range for smoother interpolation.
    ...
    n_points : int
        Number of points before and after bad range to use for interpolation
    ...
    # Find the start and end indices of each contiguous bad band range
    bad_ranges_indices = []
    start_idx = None
    for i in range(len(bad_bands_mask)):
        if bad_bands_mask[i] and start_idx is None:
            start_idx = i
        elif not bad_bands_mask[i] and start_idx is not None:
            bad_ranges_indices.append((start_idx, i - 1))
            start_idx = None
    # Handle the case where the last band is bad
    if start_idx is not None:
        bad_ranges_indices.append((start_idx, len(bad_bands_mask) - 1))
    
    print(f"Identified {len(bad_ranges_indices)} contiguous bad band ranges")
    ...
    for chunk_idx in tqdm(range(num_chunks), desc="Processing row chunks"):
        ...
        for r in range(start_row, end_row):
            for c in range(cols):
                ...
                # Process each bad range separately
                for bad_start_idx, bad_end_idx in bad_ranges_indices:
                    # Determine range of bands to use for interpolation
                    interp_start_idx = max(0, bad_start_idx - n_points)
                    interp_end_idx = min(num_bands - 1, bad_end_idx + n_points)
                    
                    # Get local bands for interpolation (excluding bad bands and nodata)
                    local_good_mask = np.ones(num_bands, dtype=bool)
                    local_good_mask[bad_start_idx:bad_end_idx + 1] = False  # Exclude the bad range
                    local_good_mask &= ~nodata_mask  # Exclude nodata values
                    
                    # Further restrict to bands within our window of n_points before and after
                    window_mask = np.zeros(num_bands, dtype=bool)
                    window_mask[interp_start_idx:interp_end_idx + 1] = True
                    local_good_mask &= window_mask
                    
                    # If we don't have enough good bands for interpolation (need at least 2), skip
                    if np.sum(local_good_mask) < 2:
                        continue
                    
                    # Get wavelengths and values for selected good bands
                    good_wavelengths = wavelengths[local_good_mask]
                    good_values = spectrum[local_good_mask]
                    
                    # Create interpolation function using only the selected points
                    try:
                        ...
                        # Get the wavelengths to interpolate (bad bands in this range that aren't nodata)
                        interp_mask = np.zeros(num_bands, dtype=bool)
                        interp_mask[bad_start_idx:bad_end_idx + 1] = True
                        interp_mask &= ~nodata_mask
...
def main():
    ...
    parser.add_argument('--output', default='AVIRIS_SWIR_INTP_SMOOTH', help='Output directory')
