...
for csv_index, csvfile in ipairs(all_csvs) do
    ...
    for shape_idx = 1, num_shapes do
        ...
        -- Print polygon coordinates if verbose
        if verbose then
            print(string.format("\nShape #%d polygon coordinates:", shape_idx))
            for vi = 1, #outer_poly, 2 do
                print(string.format("  (%.17g, %.17g)", outer_poly[vi], outer_poly[vi+1]))
            end
        end
