--------------------------------------------------------------------------------
-- metasurface_progress_centered.lua
--
-- Changes:
--   1) Only generate a single shape for each shape_idx, reused for all CSV files.
--   2) base_outer = 0.25, rand_outer = 0.2
--   3) S:SetNumG(80) instead of 40
--   4) Lattice size clarified as 1×1, polygon shifted to center at (0.5, 0.5).
--------------------------------------------------------------------------------
...
--    After generating the shape in the range ~ [-r..r], shift to (0.5, 0.5).
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
-- 8) Main loop
--    We now first pick a shape, then apply it to ALL CSV files
--    so that the shape is the same for each c value.
------------------------------------------------------------------------------
for shape_idx = 1, num_shapes do
    -- Generate a single polygon for this shape_idx
    ...
    -- For each CSV file (c value), we reuse the same polygon
    for csv_index, csvfile in ipairs(all_csvs) do
        local data = read_csv(csvfile)

        -- Attempt to parse a unique matname from CSV filename
