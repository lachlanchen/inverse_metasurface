...
class CompressionModel(nn.Module):
    ...
    def __init__(self, in_channels=100, latent_dim=11):
        ...
        self.decoder = AWAN(inplanes=latent_dim, planes=in_channels, channels=64, n_DRBs=2)
...
def main():
    ...
    # Add datetime to output directory if not explicitly provided
    if args.output_dir == 'results-simple-select':
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        args.output_dir = f"results-simple-select-{timestamp}"
