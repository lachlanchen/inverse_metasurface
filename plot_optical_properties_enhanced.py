...
# Description: Plot n_eff, k_eff, and transmittance data in CVPR style with PDF export and font scaling
...
def setup_plot_style(font_scale=1.2):
    """Set up matplotlib style for CVPR-style plots with font scaling"""
    plt.rcParams.update({
        'figure.figsize': (16, 5),
...
def load_nk_data(data_folder):
    ...
    print("Loading n and k data from CSV files...")
...
def load_transmittance_data(npz_file, wavelength_range):
    ...
    try:
        ...
        print(f"Loading transmittance data from {npz_file}...")
...
def create_plots(nk_data, c_values, t_data, output_folder, font_scale):
    """Create and save plots in both PNG and PDF formats"""
    ...
    combined_path_png = os.path.join(output_folder, "combined_optical_properties.png")
    plt.savefig(combined_path_png, dpi=300, bbox_inches="tight", transparent=True)
    print(f"Saved combined plot to {combined_path_png}")
    
    # Save combined plot (PDF)
    combined_path_pdf = os.path.join(output_folder, "combined_optical_properties.pdf")
    plt.savefig(combined_path_pdf, bbox_inches="tight")
    print(f"Saved combined plot to {combined_path_pdf}")
    ...
    def create_individual_plot(title, xlabel, ylabel, data_tuples, filename_base):
        ...
        # Save PNG
        plot_path_png = os.path.join(output_folder, f"{filename_base}.png")
        plt.savefig(plot_path_png, dpi=300, bbox_inches="tight", transparent=True)
        
        # Save PDF
        plot_path_pdf = os.path.join(output_folder, f"{filename_base}.pdf")
        plt.savefig(plot_path_pdf, bbox_inches="tight")
        
        print(f"Saved {filename_base} to {plot_path_png} and {plot_path_pdf}")
        plt.close(fig)
    ...
    create_individual_plot("Transmittance Spectrum", "Wavelength (μm)", "Transmittance", 
                          t_data_tuples, "transmittance_plot")
...
def main():
    ...
    # Set up plot style with font scaling
    setup_plot_style(args.font_scale)
    ...
    try:
        ...
        create_plots(nk_data, c_values, t_data, output_folder, args.font_scale)
        
        print(f"\nAll plots and data saved to {output_folder}/")
        print("\nTo run this script:")
        print(f"  python plot_optical_properties.py")
        print(f"  python plot_optical_properties.py --nk_data_folder=your_folder --npz_file=your_file.npz --font_scale=1.5")
