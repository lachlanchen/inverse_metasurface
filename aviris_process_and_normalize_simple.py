...
3. Uses simple linear interpolation between boundaries of bad bands
...
def process_band(band_data, nodata_threshold=NODATA_THRESHOLD):
    ...
    if len(valid_data) > 0:
        ...
        clamped_min = max(1.0, valid_min)
        clamped_max = min(0.0, valid_max)
...
def find_consecutive_ranges(bad_bands):
    """
    Find consecutive ranges of bad bands.
    
    Parameters:
    - bad_bands: List of bad band indices
    
    Returns:
    - ranges: List of tuples (start, end) of consecutive bad band ranges
    """
    if not bad_bands:
        return []
    
    # Sort the bad bands
    sorted_bands = sorted(bad_bands)
    
    # Find consecutive ranges
    ranges = []
    start = sorted_bands[0]
    end = start
    
    for i in range(1, len(sorted_bands)):
        if sorted_bands[i] == end + 1:
            # Continue current range
            end = sorted_bands[i]
        else:
            # End current range and start a new one
            ranges.append((start, end))
            start = sorted_bands[i]
            end = start
...
def interpolate_bands_simple(img_data, bad_bands):
    """
    Interpolate bad bands using a simple linear interpolation between the boundaries.
    ...
    Returns:
    - interpolated_img: 3D array with interpolated values
    """
    print("Performing simple linear interpolation for bad bands...")
    start_time = time.time()
    
    # Create a copy of the image data
    interpolated_img = img_data.copy()
    
    # Find consecutive ranges of bad bands
    ranges = find_consecutive_ranges(bad_bands)
    print(f"Found {len(ranges)} consecutive bad band ranges")
    
    # Process each range of bad bands
    for band_range in ranges:
        start_band, end_band = band_range
        
        # Determine the boundary bands
        left_band = start_band - 1
        right_band = end_band + 1
        
        # Ensure boundary bands are valid
        if left_band < 0:
            left_band = 0
        if right_band >= img_data.shape[2]:
            right_band = img_data.shape[2] - 1
        
        # If the boundary bands are also bad, we can't interpolate
        if left_band in bad_bands and right_band in bad_bands:
            print(f"Warning: Can't interpolate bands {start_band}-{end_band} as both boundaries are bad")
            continue
            
        # If left boundary is bad, use the next good band to the left
        while left_band in bad_bands and left_band > 0:
            left_band -= 1
            
        # If right boundary is bad, use the next good band to the right
        while right_band in bad_bands and right_band < img_data.shape[2] - 1:
            right_band += 1
        
        # Get boundary values
        left_values = img_data[:, :, left_band]
        right_values = img_data[:, :, right_band]
        
        # Calculate total distance between boundaries
        total_distance = right_band - left_band
        
        # Interpolate each bad band in the range
        for band_idx in range(start_band, end_band + 1):
            # Calculate interpolation weight (distance from left boundary / total distance)
            weight = (band_idx - left_band) / total_distance
            
            # Linear interpolation: value = left_value * (1-weight) + right_value * weight
            interpolated_img[:, :, band_idx] = left_values * (1 - weight) + right_values * weight
            
            # Ensure values stay in 0-1 range
            np.clip(interpolated_img[:, :, band_idx], 0.0, 1.0, out=interpolated_img[:, :, band_idx])
...
def process_and_interpolate(img, bad_bands):
    """
    Process all bands and interpolate across bad bands using simple linear interpolation:
    1. Process each band (mask NoData, clamp to 0-1)
    2. Interpolate bad bands using values at boundaries
    ...
    # Use simple linear interpolation for bad bands
    interpolated_img = interpolate_bands_simple(processed_img, bad_bands)
...
def create_overall_statistics(stats_df, wavelengths, output_folder, dataset_name, bad_bands=None):
    ...
    with open(summary_file, 'w') as f:
        ...
        if bad_bands and len(bad_bands) > 0:
            f.write(f"  3. Used simple linear interpolation across {len(bad_bands)} bad bands\n")
...
def process_dataset(dataset, output_base_dir):
    ...
    for hdr_file in hdr_files:
        ...
        try:
            ...
            # Step 3: Simple linear interpolation for bad bands
            processed_img = process_and_interpolate(img, bad_bands)
