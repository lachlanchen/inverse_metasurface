...
###############################################################################
# DECODER MODEL - Updated to 5 layers
###############################################################################
class DecoderCNN5Layer(nn.Module):
    def __init__(self):
        super().__init__()
        self.decoder = nn.Sequential(
            nn.Conv2d(11, 32, kernel_size=3, padding=1),  # 11 -> 32
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Conv2d(64, 100, kernel_size=3, padding=1)  # 64 -> 100
        )
...
class HyperspectralAutoencoder(nn.Module):
    def __init__(self, shape2filter_path, filter2shape_path, target_snr=None, initial_filter_params=None, use_random_snr=False, min_snr=0, max_snr=40):
        super().__init__()
        ...
        # Decoder: 5-layer convolutional network
        self.decoder = DecoderCNN5Layer().decoder
    ...
    def add_noise(self, tensor):
        ...
        If use_random_snr is True, a random SNR within the specified range is chosen for each batch.
        """
        if self.use_random_snr:
            # Choose a random SNR for this batch
            snr = torch.tensor(random.uniform(self.min_snr, self.max_snr), device=tensor.device)
        elif self.target_snr is None:
            return tensor
        else:
            snr = self.target_snr
...
###############################################################################
def visualize_reconstruction(model, sample_tensor, save_path, title="Reconstruction Visualization", n_wavelengths=5):
    """
    Visualize original vs reconstructed data for a sample
    
    Parameters:
    model: Trained autoencoder model
    sample_tensor: Input sample tensor [1, height, width, bands]
    save_path: Path to save the visualization
    title: Title for the plot
    n_wavelengths: Number of wavelengths to visualize
    """
    with torch.no_grad():
        # Get reconstruction
        recon, encoded = model(sample_tensor)
        
        # Convert to numpy for visualization
        original = sample_tensor.detach().cpu().numpy()[0]
        reconstructed = recon.detach().cpu().numpy()[0]
        encoded_np = encoded.detach().cpu().numpy()[0]
        
        # Select evenly spaced wavelengths to visualize
        wavelength_indices = np.linspace(0, original.shape[-1]-1, n_wavelengths, dtype=int)
        
        # Create figure with multiple subplots
        fig, axes = plt.subplots(3, n_wavelengths, figsize=(n_wavelengths*3, 9))
        ...
        for i, idx in enumerate(wavelength_indices):
            im = axes[0, i].imshow(original[:, :, idx], cmap=cmap)
            axes[0, i].set_title(f"Original λ{idx}")
            axes[0, i].axis('off')
            plt.colorbar(im, ax=axes[0, i], fraction=0.046, pad=0.04)
        
        # Plot reconstructed data
        for i, idx in enumerate(wavelength_indices):
            im = axes[1, i].imshow(reconstructed[:, :, idx], cmap=cmap)
            axes[1, i].set_title(f"Recon λ{idx}")
            axes[1, i].axis('off')
            plt.colorbar(im, ax=axes[1, i], fraction=0.046, pad=0.04)
        
        # Plot absolute difference
        for i, idx in enumerate(wavelength_indices):
            diff = np.abs(original[:, :, idx] - reconstructed[:, :, idx])
            im = axes[2, i].imshow(diff, cmap='hot')
            axes[2, i].set_title(f"Diff λ{idx}")
            axes[2, i].axis('off')
...
def visualize_spectral_profiles(model, sample_tensor, save_path, n_profiles=5, title="Spectral Profiles Comparison"):
    """
    Visualize spectral profiles of original vs reconstructed data
    
    Parameters:
    model: Trained autoencoder model
    sample_tensor: Input sample tensor [1, height, width, bands]
    save_path: Path to save the visualization
    n_profiles: Number of random pixel locations to sample
    title: Title for the plot
    """
    with torch.no_grad():
        # Get reconstruction
        ...
        for _ in range(n_profiles):
            ...
            sampled_locations.append((y, x))
        
        # Plot spectral profiles
        wavelength_indices = np.arange(bands)
        markers = ['o', 's', '^', 'd', 'x']
        colors = ['b', 'g', 'r', 'c', 'm']
        
        for i, (y, x) in enumerate(sampled_locations):
            orig_profile = original[y, x, :]
            recon_profile = reconstructed[y, x, :]
            
            marker = markers[i % len(markers)]
            color = colors[i % len(colors)]
            
            # Only use markers at a few points to avoid cluttering
            marker_indices = np.linspace(0, bands-1, 10, dtype=int)
            
            # Original profile - solid line with markers
            plt.plot(wavelength_indices, orig_profile, color=color, linestyle='-', 
                     label=f'Original ({y},{x})' if i == 0 else f'Original ({y},{x})')
            plt.plot(marker_indices, orig_profile[marker_indices], color=color, marker=marker, linestyle='none')
            
            # Reconstructed profile - dashed line
            plt.plot(wavelength_indices, recon_profile, color=color, linestyle='--', 
                     label=f'Recon ({y},{x})' if i == 0 else f'Recon ({y},{x})')
...
# TRAINING FUNCTIONS
###############################################################################
def train_with_random_noise(shape2filter_path, filter2shape_path, output_dir, 
                           min_snr=0, max_snr=40, initial_filter_params=None, 
                           batch_size=10, num_epochs=500, learning_rate=0.001, 
                           cache_file=None, use_cache=False, folder_patterns="all",
                           train_data=None, test_data=None):
    """
    Train and visualize the hyperspectral autoencoder with random noise levels
    ...
    """
    # Create output directory with random noise indicator
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    noise_dir = f"random_noise_{min_snr}to{max_snr}dB"
    ...
    # Create subfolder for reconstruction visualizations
    ...
    # Initialize model with random noise settings
    model = HyperspectralAutoencoder(
        shape2filter_path, filter2shape_path, 
        use_random_snr=True, min_snr=min_snr, max_snr=max_snr,
        initial_filter_params=initial_filter_params
    ).to(device)
    
    print(f"Model initialized with random noise levels between {min_snr} dB and {max_snr} dB")
    ...
    plt.title(f"Initial Spectral Parameters (Random Noise: {min_snr}-{max_snr} dB)")
    plt.legend()
    ...
    plt.xlabel("Wavelength Index")
    plt.ylabel("Filter Value")
    plt.title(f"Initial Reconstructed Filter (Random Noise: {min_snr}-{max_snr} dB)")
    plt.legend()
    ...
    initial_recon_path = f"{recon_dir}/initial_reconstruction.png"
    initial_train_mse = visualize_reconstruction(
        model, sample_tensor, 
        initial_recon_path, 
        title=f"Initial Reconstruction (Random Noise: {min_snr}-{max_snr} dB)"
    )
    print(f"Initial Train MSE: {initial_train_mse:.6f}")
    
    # Visualize initial spectral profiles
    initial_profile_path = f"{recon_dir}/initial_spectral_profiles.png"
    visualize_spectral_profiles(
        model, sample_tensor, 
        initial_profile_path, 
        title=f"Initial Spectral Profiles (Random Noise: {min_snr}-{max_snr} dB)"
    )
    
    # If test data is available, visualize test reconstruction
    initial_test_mse = None
    if test_sample_tensor is not None:
        initial_test_recon_path = f"{recon_dir}/initial_test_reconstruction.png"
        initial_test_mse = visualize_reconstruction(
            model, test_sample_tensor, 
            initial_test_recon_path, 
            title=f"Initial Test Reconstruction (Random Noise: {min_snr}-{max_snr} dB)"
        )
    ...
    print(f"Starting training with random noise between {min_snr} dB and {max_snr} dB...")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % (num_epochs // 10) == 0 or epoch == 0:
            ...
            recon_path = f"{recon_dir}/reconstruction_epoch_{epoch+1}.png"
            ...
            profile_path = f"{recon_dir}/spectral_profiles_epoch_{epoch+1}.png"
            visualize_spectral_profiles(
                model, sample_tensor, 
                profile_path, 
                title=f"Spectral Profiles at Epoch {epoch+1}"
            )
    ...
    plt.xlabel("Wavelength Index")
    plt.ylabel("Filter Value")
    plt.title(f"Final Spectral Parameters (Random Noise: {min_snr}-{max_snr} dB)")
    plt.legend()
    ...
    plt.xlabel("Wavelength Index")
    plt.ylabel("Filter Value")
    plt.title(f"Final Reconstructed Filter (Random Noise: {min_snr}-{max_snr} dB)")
    plt.legend()
    ...
    final_recon_path = f"{recon_dir}/final_reconstruction.png"
    final_train_mse = visualize_reconstruction(
        model, sample_tensor, 
        final_recon_path, 
        title=f"Final Reconstruction (Random Noise: {min_snr}-{max_snr} dB)"
    )
    print(f"Final Train MSE: {final_train_mse:.6f}")
    
    # Visualize final spectral profiles
    final_profile_path = f"{recon_dir}/final_spectral_profiles.png"
    visualize_spectral_profiles(
        model, sample_tensor, 
        final_profile_path, 
        title=f"Final Spectral Profiles (Random Noise: {min_snr}-{max_snr} dB)"
    )
    
    # If test data is available, visualize test reconstruction
    final_test_mse = None
    if test_sample_tensor is not None:
        final_test_recon_path = f"{recon_dir}/final_test_reconstruction.png"
        final_test_mse = visualize_reconstruction(
            model, test_sample_tensor, 
            final_test_recon_path, 
            title=f"Final Test Reconstruction (Random Noise: {min_snr}-{max_snr} dB)"
        )
    ...
    plt.title(f"Filter Matrix Condition Number During Training (Random Noise: {min_snr}-{max_snr} dB)")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title(f"Training Loss (Random Noise: {min_snr}-{max_snr} dB)")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE")
    plt.title(f"MSE of Reconstruction and Input (Random Noise: {min_snr}-{max_snr} dB)")
    plt.legend()
    ...
    with open(f"{output_dir}/training_params.txt", "w") as f:
        f.write(f"Noise level (SNR): Random {min_snr}-{max_snr} dB\n")
    ...
    print(f"Training with random noise ({min_snr}-{max_snr} dB) completed. All results saved to {output_dir}/")
...
def train_decoder_with_comparison(shape2filter_path, filter2shape_path, shapes_dict, output_dir, noise_levels, 
                                 num_epochs=100, batch_size=10, learning_rate=0.001, 
                                 cache_file=None, use_cache=False, folder_patterns="all", 
                                 train_data=None, test_data=None):
    """
    Train decoders for all shapes (initial, lowest MSE, lowest CN, final) with different noise levels
    ...
    output_dir: Directory to save outputs
    noise_levels: List of SNR levels in dB to test
    num_epochs: Number of training epochs
    batch_size: Batch size for training
    learning_rate: Learning rate for optimizer
    ...
    decoder_dir = os.path.join(output_dir, f"decoder_comparison")
    os.makedirs(decoder_dir, exist_ok=True)
    print(f"Saving decoder comparison results to {decoder_dir}/")
    
    # Create a folder for reconstruction visualizations
    recon_dir = os.path.join(decoder_dir, "reconstructions")
    os.makedirs(recon_dir, exist_ok=True)
    ...
    # Create a dictionary to store results for each noise level
    all_results = {}
    
    # Loop over each noise level
    for noise_level in noise_levels:
        print(f"\nTraining decoders for noise level: {noise_level} dB")
        
        # Create a directory for this noise level
        noise_dir = os.path.join(decoder_dir, f"noise_{noise_level}dB")
        os.makedirs(noise_dir, exist_ok=True)
        
        # Create a folder for visualization at this noise level
        noise_recon_dir = os.path.join(recon_dir, f"noise_{noise_level}dB")
        ...
        for shape_type in shapes:
            # Initialize model with fixed noise level
            models[shape_type] = HyperspectralAutoencoder(
                shape2filter_path, filter2shape_path, 
                target_snr=noise_level
            ).to(device)
            ...
            with torch.no_grad():
                ...
                # Visualize initial reconstruction
                initial_recon_path = f"{noise_recon_dir}/{shape_type}_initial_reconstruction.png"
                visualize_reconstruction(
                    models[shape_type], train_sample, 
                    initial_recon_path, 
                    title=f"Initial {shape_type.capitalize()} Shape Reconstruction (SNR: {noise_level} dB)"
                )
                
                # Visualize initial spectral profiles
                initial_profile_path = f"{noise_recon_dir}/{shape_type}_initial_spectral_profiles.png"
                visualize_spectral_profiles(
                    models[shape_type], train_sample, 
                    initial_profile_path, 
                    title=f"Initial {shape_type.capitalize()} Shape Spectral Profiles (SNR: {noise_level} dB)"
                )
        ...
        for epoch in range(num_epochs):
            ...
            # Visualize reconstructions at regular intervals
            if (epoch + 1) % (num_epochs // 4) == 0:
                for shape_type in shapes:
                    # Visualize reconstruction
                    recon_path = f"{noise_recon_dir}/{shape_type}_reconstruction_epoch_{epoch+1}.png"
                    visualize_reconstruction(
                        models[shape_type], train_sample, 
                        recon_path, 
                        title=f"{shape_type.capitalize()} Shape Reconstruction at Epoch {epoch+1} (SNR: {noise_level} dB)"
                    )
        
        # After training, create final visualizations
        for shape_type in shapes:
            # Visualize final reconstruction
            final_recon_path = f"{noise_recon_dir}/{shape_type}_final_reconstruction.png"
            visualize_reconstruction(
                models[shape_type], train_sample, 
                final_recon_path, 
                title=f"Final {shape_type.capitalize()} Shape Reconstruction (SNR: {noise_level} dB)"
            )
            
            # Visualize final spectral profiles
            final_profile_path = f"{noise_recon_dir}/{shape_type}_final_spectral_profiles.png"
            visualize_spectral_profiles(
                models[shape_type], train_sample, 
                final_profile_path, 
                title=f"Final {shape_type.capitalize()} Shape Spectral Profiles (SNR: {noise_level} dB)"
            )
        
        # Create plots directory for this noise level
        plots_dir = os.path.join(noise_dir, "plots")
        os.makedirs(plots_dir, exist_ok=True)
        
        # Plot combined MSE curves for all shape types - Final values
        plt.figure(figsize=(12, 8))
        ...
        # Save the MSE values
        for shape_type in shapes:
            np.save(f"{noise_dir}/{shape_type}_train_mse.npy", np.array(mse_values[shape_type]['train']))
            np.save(f"{noise_dir}/{shape_type}_test_mse.npy", np.array(mse_values[shape_type]['test']))
        ...
        for shape_type in shapes:
            ...
            # Save final model
            torch.save(models[shape_type].decoder.state_dict(), f"{noise_dir}/{shape_type}_final_decoder_state.pt")
            
            # Save best model
            if best_decoders[shape_type] is not None:
                torch.save(best_decoders[shape_type], f"{noise_dir}/{shape_type}_best_decoder_state.pt")
        
        # Create log file to save results
        with open(f"{noise_dir}/decoder_comparison_results.txt", "w") as f:
        ...
        print(f"Decoder comparison for noise level {noise_level} dB completed.")
    
    # Create summary plots directory
    summary_dir = os.path.join(decoder_dir, "summary_plots")
    os.makedirs(summary_dir, exist_ok=True)
    
    # Collect results for all noise levels - both final and best MSE
    final_train_results = {shape_type: [] for shape_type in shapes}
    final_test_results = {shape_type: [] for shape_type in shapes}
    best_train_results = {shape_type: [] for shape_type in shapes}
    best_test_results = {shape_type: [] for shape_type in shapes}
    
    for noise_level in noise_levels:
        for shape_type in shapes:
            # Add final MSE values
            final_train_results[shape_type].append(all_results[noise_level]['final_mse'][shape_type]['train'])
            final_test_results[shape_type].append(all_results[noise_level]['final_mse'][shape_type]['test'])
            
            # Add best MSE values
            best_train_results[shape_type].append(all_results[noise_level]['best_mse'][shape_type]['train'])
            best_test_results[shape_type].append(all_results[noise_level]['best_mse'][shape_type]['test'])
    ...
    print(f"All decoder comparisons completed. Results saved to: {decoder_dir}/")
...
def main():
    ...
    parser.add_argument("--min-snr", type=float, default=0, help="Minimum SNR level in dB for random noise")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_dir = f"random_noise_experiment_{folder_suffix}_{timestamp}"
    ...
    # Define noise levels to test in decoder-only stage
    noise_levels = [0, 5, 10, 15, 20, 25, 30, 35, 40]
    
    # Run single training with random noise between min_snr and max_snr
    print(f"\n{'='*50}")
    print(f"Running joint training with random noise between {args.min_snr} dB and {args.max_snr} dB")
    print(f"{'='*50}\n")
    
    initial_shape_path, lowest_mse_shape_path, lowest_cn_shape_path, final_shape_path, output_dir = train_with_random_noise(
    ...
    # Run decoder comparison training for all shapes at multiple fixed noise levels
    print(f"\n{'='*50}")
    print(f"Running decoder comparison training for all shapes at multiple noise levels")
    print(f"{'='*50}\n")
    
    # Train decoder with comparison of all shapes at various noise levels
    mse_results = train_decoder_with_comparison(
        shape2filter_path=shape2filter_path,
        filter2shape_path=filter2shape_path,
        shapes_dict=shape_paths,
