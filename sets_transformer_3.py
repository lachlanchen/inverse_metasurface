...
################################################################################
# 2. Model Definition (Deep Sets + LSTM Decoder)
################################################################################
...
class LSTMDecoder(nn.Module):
    ...
    def forward(self, encoding):
        ...
        lstm_out, _ = self.lstm(x_in, (h0, c0))
...
################################################################################
# 3. Utility Functions: Loss, Sorting, Plotting, etc.
################################################################################
...
def angle_sort(points):
    """
    Sort a list of (x, y) points by their angle from the origin, ascending.
    This helps ensure polygons are drawn in a sensible order.
    """
    # Convert list of tuples -> array for convenience
    arr = np.array(points)
    if arr.shape[0] == 0:
        return []
    
    # angles
    angles = np.arctan2(arr[:,1], arr[:,0])  # angle from x-axis
    sorted_idx = np.argsort(angles)
    sorted_pts = arr[sorted_idx]
    return sorted_pts.tolist()

def c4_symmetry(q1_points):
    """
    Given first-quadrant points, replicate them with C4 symmetry around origin.
    Returns a list of (x, y) with Q1, Q2, Q3, Q4.
    """
    full_polygon = []
    for (x, y) in q1_points:
        # Q1
        full_polygon.append((x, y))
        # Q2
        full_polygon.append((-y, x))
        # Q3
        full_polygon.append((-x, -y))
        # Q4
        full_polygon.append((y, -x))
...
def save_polygon_csv(path, points):
    """
    Save a list of (x, y) points to CSV with columns x,y
    """
...
def plot_polygons(gt_points, pred_points, out_path, title="Polygon Comparison"):
    """
    Plot GT vs predicted polygons in the same figure.
    """
    plt.figure(figsize=(6,6))
    
    if len(gt_points) > 1:
        # sort by angle
        gt_sorted = angle_sort(gt_points)
        gt_x = [pt[0] for pt in gt_sorted]
        gt_y = [pt[1] for pt in gt_sorted]
        # close the polygon
        gt_x.append(gt_sorted[0][0])
        gt_y.append(gt_sorted[0][1])
        plt.plot(gt_x, gt_y, 'b--', marker='o', label='GT Polygon')
    else:
        # single point or empty
        for pt in gt_points:
            plt.plot(pt[0], pt[1], 'bo', label='GT Vertex')
    
    if len(pred_points) > 1:
        pred_sorted = angle_sort(pred_points)
        px = [pt[0] for pt in pred_sorted]
        py = [pt[1] for pt in pred_sorted]
        px.append(pred_sorted[0][0])
        py.append(pred_sorted[0][1])
        plt.plot(px, py, 'r-', marker='x', label='Pred Polygon')
    else:
        for pt in pred_points:
            plt.plot(pt[0], pt[1], 'rx', label='Pred Vertex')
...
################################################################################
# 4. Evaluation: presence accuracy, coordinate MSE, and polygon saving/plotting
################################################################################
...
def evaluate_model(model, dataloader, device, out_dir):
    ...
    and also save each predicted polygon (including C4 symmetry) + GT polygon into out_dir/polygons.
    We'll also plot GT vs predicted polygons (both extended by C4).
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (R_data_np, label_np, uid) in enumerate(dataloader):
            ...
            # --------- 1) Presence Accuracy ---------
            p_sig = torch.sigmoid(p_pred)  # (bsz, 4), in [0,1]
            ...
            # --------- 3) Save & Plot Polygons ---------
            bsz = preds.size(0)
            for i in range(bsz):
                sample_uid = uid[i]
                
                # 3.1 GT polygon from label
                gt_presence = p_tgt[i].cpu().numpy()
                gt_xy = xy_tgt[i].cpu().numpy()
                q1_gt_points = []
                for v in range(4):
                    if gt_presence[v] > 0.5:
                        q1_gt_points.append((gt_xy[v,0], gt_xy[v,1]))
                # c4 expand
                gt_full = c4_symmetry(q1_gt_points)
                
                # 3.2 Pred polygon
                pred_presence = p_sig[i].cpu().numpy()  # continuous, e.g. 0.7
                pred_xy = xy_pred[i].cpu().numpy()
                q1_pred_points = []
                for v in range(4):
                    if pred_presence[v] > 0.5:
                        xval = pred_xy[v,0]
                        yval = pred_xy[v,1]
                        q1_pred_points.append((xval, yval))
                pred_full = c4_symmetry(q1_pred_points)
                
                # 3.3 Save CSV for GT + predicted
                # We'll store presence bits + coords for Q1 in one file, 
                # plus the C4 expansions in another, or together. 
                # For clarity, let's just store the final expansions as well.
                
                # GT Q1
                q1_gt_csv_path = os.path.join(poly_dir, f"{sample_uid}_gt_q1.csv")
                with open(q1_gt_csv_path, "w") as f:
                    f.write("presence,x,y\n")
                    for v in range(4):
                        f.write(f"{gt_presence[v]:.0f},{gt_xy[v,0]:.6f},{gt_xy[v,1]:.6f}\n")
                
                # Pred Q1
                q1_pred_csv_path = os.path.join(poly_dir, f"{sample_uid}_pred_q1.csv")
                with open(q1_pred_csv_path, "w") as f:
                    f.write("presence,x,y\n")
                    for v in range(4):
                        f.write(f"{pred_presence[v]:.3f},{pred_xy[v,0]:.6f},{pred_xy[v,1]:.6f}\n")
                
                # GT C4
                c4_gt_csv_path = os.path.join(poly_dir, f"{sample_uid}_gt_c4.csv")
                save_polygon_csv(c4_gt_csv_path, gt_full)
                
                # Pred C4
                c4_pred_csv_path = os.path.join(poly_dir, f"{sample_uid}_pred_c4.csv")
                save_polygon_csv(c4_pred_csv_path, pred_full)
                
                # 3.4 Plot side-by-side or on one figure
                plot_path = os.path.join(poly_dir, f"{sample_uid}_polygon.png")
                plot_polygons(gt_full, pred_full, plot_path,
                              title=f"Polygon (C4) - UID={sample_uid}")
...
def train_model(csv_file="merged_s4_shapes.csv", 
    ...
    # 5.7. Evaluate on test set: presence accuracy & coordinate MSE
