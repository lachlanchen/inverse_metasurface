...
def save_plot_data(data_dict, filename):
    """
    Save the data used in a plot for later use
    
    Parameters:
    data_dict: Dictionary containing data used in the plot
    filename: Path to save the data
    """
    os.makedirs(os.path.dirname(os.path.abspath(filename)), exist_ok=True)
    np.savez(filename, **data_dict)
    print(f"Plot data saved to: {os.path.abspath(filename)}")

# Set ICCP paper style for plots
def set_iccp_paper_style():
    """Set plot style according to ICCP paper guidelines"""
    # Use default style as base and customize from there
    ...
    plt.rcParams.update({
        ...
        'axes.grid': False,  # No grid for ICCP style
...
def plot_shape_with_c4(shape, title, save_path):
    ...
    # Use ICCP paper style
    set_iccp_paper_style()
    ...
    plt.scatter(shape[presence, 1], shape[presence, 2], color='red', s=50, label='Control Points')
    ...
    # Save the data used in the plot
    plot_data = {
        'shape': shape,
        'title': title
    }
    save_plot_data(plot_data, save_path.replace('.png', '_data.npz'))
...
def train_with_noise_level(model_path, output_dir, noise_level, batch_size=10, num_epochs=500, 
    ...
    Returns:
    tuple: (initial_shape_path, least_cn_shape_path, final_shape_path, output_dir)
    """
    ...
    save_plot_data(filter_data, initial_filters_path.replace('.png', '_data.npz'))
    ...
    # Variables to track the shape with the lowest condition number
    lowest_condition_number = initial_condition_number
    lowest_cn_epoch = 0
    lowest_cn_shape = initial_shape.copy()
    lowest_cn_filters = initial_filters.clone()
    lowest_cn_raw_params = model.raw_params.detach().clone()
    lowest_cn_raw_shift = model.raw_shift.detach().clone()
    ...
    for epoch in range(num_epochs):
        ...
        if current_condition_number < lowest_condition_number:
            ...
            lowest_cn_epoch = epoch + 1
            lowest_cn_shape = model.get_current_shape().detach().cpu().numpy()
            lowest_cn_filters = current_filters.clone()
            lowest_cn_raw_params = model.raw_params.detach().clone()
            lowest_cn_raw_shift = model.raw_shift.detach().clone()
            print(f"New lowest condition number: {lowest_condition_number:.4f} at epoch {lowest_cn_epoch}")
    ...
    # Save shape with lowest condition number
    least_cn_shape_path = f"{output_dir}/lowest_cn_shape_epoch_{lowest_cn_epoch}.png"
    plot_shape_with_c4(lowest_cn_shape, f"Lowest CN Shape (Epoch {lowest_cn_epoch}, CN: {lowest_condition_number:.4f})", least_cn_shape_path)
    print(f"Lowest CN shape saved to: {os.path.abspath(least_cn_shape_path)}")
    
    # Save lowest CN filters
    set_iccp_paper_style()
    plt.figure(figsize=(6, 4))
    for i in range(11):
    ...
    plt.xlabel("Wavelength Index")
    plt.ylabel("Filter Value")
    plt.title(f"Lowest CN Spectral Filters (Epoch {lowest_cn_epoch}, CN: {lowest_condition_number:.4f})")
    plt.legend(frameon=False)
    lowest_cn_filters_path = f"{output_dir}/lowest_cn_filters.png"
    plt.savefig(lowest_cn_filters_path, bbox_inches='tight')
    plt.close()
    
    # Save lowest CN filter data
    lowest_cn_filter_data = {
        'filters': lowest_cn_filters.numpy(),
        'condition_number': lowest_condition_number,
        'epoch': lowest_cn_epoch
    }
    save_plot_data(lowest_cn_filter_data, lowest_cn_filters_path.replace('.png', '_data.npz'))
    
    # Save lowest CN model parameters separately
    lowest_cn_model_path = f"{output_dir}/lowest_cn_model_state_epoch_{lowest_cn_epoch}.pt"
    torch.save({
        'raw_params': lowest_cn_raw_params.cpu(),  # Fixed: added .cpu()
        'raw_shift': lowest_cn_raw_shift.cpu(),    # Fixed: added .cpu()
        'condition_number': lowest_condition_number,
        'epoch': lowest_cn_epoch
    }, lowest_cn_model_path)
    
    # Save lowest CN shape as NPY for later use
    np.save(f"{output_dir}/lowest_cn_shape_epoch_{lowest_cn_epoch}.npy", lowest_cn_shape)
    ...
    # Save final filter data
    final_filter_data = {
        'filters': final_filters.numpy(),
        'condition_number': final_condition_number
    }
    save_plot_data(final_filter_data, final_filters_path.replace('.png', '_data.npz'))
    ...
    condition_data = {
        ...
        'lowest_cn_epoch': lowest_cn_epoch,
        'lowest_condition_number': lowest_condition_number
    }
    save_plot_data(condition_data, condition_plot_path.replace('.png', '_data.npz'))
    ...
    save_plot_data(loss_data, loss_plot_path.replace('.png', '_data.npz'))
    ...
    save_plot_data(mse_data, mse_plot_path.replace('.png', '_data.npz'))
    ...
    np.save(f"{output_dir}/lowest_cn_shape.npy", lowest_cn_shape)
    np.save(f"{output_dir}/final_shape.npy", final_shape)
    np.save(f"{output_dir}/initial_filters.npy", initial_filters.numpy())
    np.save(f"{output_dir}/lowest_cn_filters.npy", lowest_cn_filters.numpy())
    ...
    with open(f"{output_dir}/training_params.txt", "w") as f:
        ...
        f.write(f"Lowest condition number shape parameters (epoch {lowest_cn_epoch}):\n")
        f.write(f"raw_params:\n{lowest_cn_raw_params.cpu().numpy()}\n")  # Fixed: added .cpu()
        f.write(f"raw_shift: {lowest_cn_raw_shift.cpu().item()}\n\n")     # Fixed: added .cpu()
        ...
        f.write(f"Lowest condition number: {lowest_condition_number:.4f} (epoch {lowest_cn_epoch})\n")
        f.write(f"Final condition number: {final_condition_number:.4f}\n")
        f.write(f"Condition number change (initial to lowest): {lowest_condition_number - initial_condition_number:.4f}\n")
        f.write(f"Condition number change (initial to final): {final_condition_number - initial_condition_number:.4f}\n\n")
    ...
    # Return paths to the initial, lowest CN, and final shapes for later training
    initial_shape_npy_path = f"{output_dir}/initial_shape.npy"
    lowest_cn_shape_npy_path = f"{output_dir}/lowest_cn_shape.npy"
    final_shape_npy_path = f"{output_dir}/final_shape.npy"
    return initial_shape_npy_path, lowest_cn_shape_npy_path, final_shape_npy_path, output_dir
...
def train_decoder_only(model_path, shape_path, output_dir, noise_level, num_epochs=100, 
    ...
    """
    # Determine shape type from path
    if "lowest_cn" in shape_path:
        shape_type = "lowest_cn"
    elif "initial" in shape_path:
        shape_type = "initial"
    else:
        shape_type = "final"
...
def plot_noise_comparison(noise_levels, initial_mse_values, lowest_cn_mse_values, final_mse_values, output_dir):
    """
    Plot the comparison of MSE values for initial, lowest condition number, and final shapes across different noise levels
    """
    set_iccp_paper_style()
    ...
    plt.plot(noise_levels, lowest_cn_mse_values, 'o-', color='green', label='Lowest CN Shape')
    plt.plot(noise_levels, final_mse_values, 'o-', color='blue', label='Final Shape')
    
    # Add labels
    plt.xlabel("Noise Level (SNR in dB)")
    plt.ylabel("MSE after Decoder Training")
    ...
    results_data = {
        ...
        'lowest_cn_mse_values': lowest_cn_mse_values,
        'final_mse_values': final_mse_values
    }
    save_plot_data(results_data, comparison_path.replace('.png', '_data.npz'))
...
def main():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_dir = f"spectral_filter_noise_experiment_{timestamp}"
    ...
    noise_levels = [0, 10, 20, 30, 40]  # Using fewer levels for faster execution
    
    # Dictionary to store paths to initial, lowest CN, and final shapes for each noise level
    shape_paths = {}
    ...
    for noise_level in noise_levels:
        ...
        initial_shape_path, lowest_cn_shape_path, final_shape_path, output_dir = train_with_noise_level(
    ...
    for noise_level in noise_levels:
        ...
        # Get paths to initial, lowest CN, and final shapes
        ...
        # Train decoder with lowest CN shape
        print(f"\nTraining decoder with lowest CN shape, SNR: {noise_level} dB")
        lowest_cn_mse = train_decoder_only(
            model_path=model_path,
            shape_path=paths['lowest_cn'],
            output_dir=decoder_output_dir,
            noise_level=noise_level,
            num_epochs=args.decoder_epochs,
            batch_size=args.batch_size,
        ...
        lowest_cn_mse_values.append(lowest_cn_mse)
    ...
    # Plot comparison of decoder-only training results for all three shape types
    ...
    with open(f"{base_output_dir}/experiments_summary.txt", "w") as f:
        f.write(f"Spectral Filter Shape Optimization Experiments Summary\n")
        f.write(f"{'='*50}\n\n")
        f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Model path: {model_path}\n")
        f.write(f"Batch size: {args.batch_size}\n")
        f.write(f"Training epochs: {args.epochs}\n")
        f.write(f"Decoder-only epochs: {args.decoder_epochs}\n")
        f.write(f"Learning rate: {args.learning_rate}\n\n")
        
        f.write(f"Noise Levels and MSE Results:\n")
        f.write(f"{'-'*50}\n")
        f.write(f"{'SNR (dB)':<10} {'Initial MSE':<15} {'Lowest CN MSE':<15} {'Final MSE':<15}\n")
        
        for i, noise_level in enumerate(noise_levels):
            f.write(f"{noise_level:<10} {initial_mse_values[i]:<15.6f} {lowest_cn_mse_values[i]:<15.6f} {final_mse_values[i]:<15.6f}\n")
