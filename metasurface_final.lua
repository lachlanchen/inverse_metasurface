...
-- It accepts SEVEN main arguments, in order:
--   1) N_quarter   (integer, e.g. 4 => polygon has 4*N_quarter vertices)
--   2) num_shapes  (integer, e.g. number of shapes to generate)
--   3) random_seed (integer, used to set math.randomseed for reproducibility)
--   4) prefix      (string, if empty => generate a datetime prefix and do not resume,
--                   otherwise attempt to resume if the CSV file exists)
--   5) num_g       (integer, used for S:SetNumG(...) to set plane-wave expansions)
--   6) base_outer  (float, the base radius for polygon generation)
--   7) rand_outer  (float, the random range ± around base_outer)
--
-- Optional flags (after the 7th argument):
--   -v  => verbose mode
--   -s  => store_data => if set, writes CSV lines to an output file (appending if resume)
--
-- This script reads all CSV files from "partial_crys_data/" exactly once (to speed up
-- repeated disk I/O), storing them in memory. Then, for each shape and each CSV, it sets
-- up an S4 simulation, calculates reflection/transmission (R, T), and writes results.
-- If 'resume' mode is enabled (prefix != ""), it attempts to parse the last line of
-- the existing results file to skip any completed shape/csv/row combos.
--------------------------------------------------------------------------------
...
-- Grab the S4.arg string from the S4 environment
local arg_str = S4.arg
if arg_str then
    -- Tokenize: split the string by whitespace
    local tokens = {}
...
-- Safety check: don't allow N_quarter < 1
if N_quarter < 1 then
...
-- Print final arguments for debugging/logging
...
------------------------------------------------------------------------------
-- 1) CSV listing
--    We gather all CSV filenames from the "partial_crys_data" folder.
--    We'll read them all into memory (see below) to avoid re-reading from disk.
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
-- 2) read_csv once in memory
--    We'll define a function that reads a single CSV file's lines, storing
--    (wavelength, n_eff, k_eff) in a table. Then we store that table in data_cache.
------------------------------------------------------------------------------
...
-- data_cache will hold all CSV data in memory
local data_cache = {}
...
------------------------------------------------------------------------------
-- 3) randomseed
--    We set the math.randomseed for reproducible shape generation.
------------------------------------------------------------------------------
math.randomseed(random_seed)

------------------------------------------------------------------------------
-- 4) Generate polygon
--    This function builds a C4-symmetric polygon with N = 4*N_quarter vertices,
--    randomizing the radius in [base_r - rand_amt, base_r + rand_amt] for each quadrant.
------------------------------------------------------------------------------
local function generate_c4_polygon(N, base_r, rand_amt)
    if (N % 4) ~= 0 then
    ...
    -- random radius array for one quadrant
    local radii = {}
    for i=1, quarter do
    ...
    -- Construct the entire polygon in a polar-like manner
    for i=0, N-1 do
    ...
    -- Shift so that (0,0) => (0.5, 0.5) in the 1×1 cell
    for i=1, #verts, 2 do
        verts[i]   = verts[i]   + 0.5
...
------------------------------------------------------------------------------
-- 6) Decide final prefix logic
--    If prefix == "", we set prefix to a datetime stamp and do_resume = false => new run
--    Otherwise, prefix is used for the final filename and we attempt to resume.
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
-- 7) Build final name with all arguments in the filename
...
------------------------------------------------------------------------------
-- 8) Resume logic
--    If store_data == true and do_resume == true, we try to parse an existing file.
--    If found with valid lines, we pick up from the last shape/csv/row. Otherwise, new.
------------------------------------------------------------------------------
local out_file = nil
...
-- For skipping, build a map from csvfile -> index in all_csvs
local csv_index_map = {}
for i, f in ipairs(all_csvs) do
    csv_index_map[f] = i
end

-- parse_resume_position: reads the last valid line from an existing CSV
...
if store_data then
    if do_resume then
        ...
        if handle then
            ...
            if info then
                resume_info = info
                out_file = io.open(out_filename, "a")  -- append mode
...
-- If we have resume_info, set the shape/csv/row so we skip completed items
if resume_info then
...
--    We sum up (#rows in each CSV) * num_shapes to get total_iterations.
------------------------------------------------------------------------------
...
-- If resuming, find how many lines are already done (minus the CSV header)
...
------------------------------------------------------------------------------
-- 10) Main loop: shapes (1..num_shapes) x CSV files
--     For each shape, we generate a new polygon, save it, then iterate CSVs,
--     skipping already completed portions if resuming. We have all CSV data in data_cache.
------------------------------------------------------------------------------
for shape_idx = resume_shape_idx, num_shapes do
    ...
    for csv_i = start_csv_index, #all_csvs do
        ...
        local data = data_cache[csvfile]  -- loaded from memory

        -- Attempt to parse a material name from the CSV file name
        ...
        for i = start_row, #data do
            ...
            -- Place the polygon in the "MetaLayer"
            S:SetLayerPatternPolygon("MetaLayer", matname, {0,0}, 0, outer_poly)

            -- Set plane wave incidence
            S:SetExcitationPlanewave({0,0}, {1,0}, {0,0})
            S:SetFrequency(freq)

            -- Compute reflection (R) and transmission (T)
            ...
            -- Write results if store_data == true
...
-- Print a blank line, then close output if we opened it
print("")
if out_file then
    out_file:close()
end
