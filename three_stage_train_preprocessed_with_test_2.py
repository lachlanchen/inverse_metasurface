...
def train_stageC_spec2shape_chain(data_source, out_dir, shape2spec_ckpt, spec2shape_ckpt, num_epochs=10,
    ...
    # IMPORTANT: In Stage C, we want the fixed shape2spec network to remain fixed
    # and allow gradients to flow from the loss (MSE between reconstructed spec and GT spec)
    # to update spec2shape_net. To do that, we do NOT wrap the fixed branch in torch.no_grad(),
    # so we set no_grad_frozen=False.
