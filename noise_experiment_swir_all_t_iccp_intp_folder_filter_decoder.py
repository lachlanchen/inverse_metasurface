...
class HyperspectralAutoencoder(nn.Module):
    def __init__(self, shape2spec_model_path, target_snr=None):
        ...
        # # Random initialization for shape parameters
        # # First column: vertex presence parameters (pre-sigmoid values)
        # # Second column: radius parameters (pre-sigmoid values)
        # self.raw_params = nn.Parameter(torch.tensor([
        #     [4.0, torch.randn(1).item()],  # First vertex always present (high value) with random radius
        #     [torch.randn(1).item(), torch.randn(1).item()],  # Random presence and radius
        #     [torch.randn(1).item(), torch.randn(1).item()],  # Random presence and radius
        #     [torch.randn(1).item(), torch.randn(1).item()]   # Random presence and radius
        # ], dtype=torch.float32))
        
        self.raw_shift = nn.Parameter(torch.tensor([0.0], dtype=torch.float32))
        # Random initialization for angle shift parameter with controlled range
        # self.raw_shift = nn.Parameter((torch.rand(1, dtype=torch.float32) * 2 - 1))  # Uniform in [-1, 1]
...
# Function to train the decoder only with a fixed shape (using a 5-layer CNN decoder)
def train_decoder_only(model_path, shape_path, output_dir, noise_level, num_epochs=100, 
                      batch_size=10, learning_rate=0.001, cache_file=None, use_cache=False, folder_patterns="all"):
    """
    Train only the decoder with a fixed shape, using a new 5-layer CNN decoder
    ...
    # Replace the original 3-layer decoder with the new 5-layer decoder
    model.decoder = new_decoder.decoder
    
    print(f"Using new 5-layer CNN decoder with fixed {shape_type} shape, SNR: {noise_level} dB")
    
    # Sample for evaluation
    sample_idx = min(5, len(data) - 1)
    sample_tensor = data[sample_idx:sample_idx+1]
    
    # Get initial MSE with the new 5-layer decoder (before training)
    with torch.no_grad():
        initial_recon, _ = model(sample_tensor)
        initial_mse = ((initial_recon - sample_tensor) ** 2).mean().item()
        print(f"Initial MSE with fixed {shape_type} shape and 5-layer decoder: {initial_mse:.6f}")
    ...
    print(f"Starting 5-layer decoder training for {shape_type} shape, SNR: {noise_level} dB...")
    ...
    with torch.no_grad():
        ...
        final_mse = ((final_recon - sample_tensor) ** 2).mean().item()
        print(f"Final MSE after 5-layer decoder training: {final_mse:.6f}")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title(f"5-Layer Decoder Training Loss ({shape_type} shape, SNR: {noise_level} dB)")
    ...
    with open(f"{decoder_dir}/training_params.txt", "w") as f:
        ...
        f.write(f"Decoder type: 5-layer CNN\n")
        f.write(f"Noise level (SNR): {noise_level} dB\n")
        f.write(f"Number of epochs: {num_epochs}\n")
        f.write(f"Learning rate: {learning_rate}\n")
        f.write(f"Folder patterns: {folder_patterns}\n")
    ...
    print(f"5-layer decoder training completed for {shape_type} shape, SNR: {noise_level} dB")
...
def main():
    ...
    for noise_level in noise_levels:
        print(f"\n{'='*50}")
        print(f"Running 5-layer decoder training for noise level: {noise_level} dB")
        print(f"{'='*50}\n")
        ...
        print(f"\nTraining 5-layer decoder with initial shape, SNR: {noise_level} dB")
        ...
        print(f"\nTraining 5-layer decoder with lowest condition number shape, SNR: {noise_level} dB")
        ...
        print(f"\nTraining 5-layer decoder with final shape, SNR: {noise_level} dB")
