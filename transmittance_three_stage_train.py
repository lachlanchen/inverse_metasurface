...
def process_csv_file(csv_path, max_points=4):
    ...
    for each group (which must have exactly 11 rows) process the transmittance
    ...
    if len(t_cols) == 0:
        raise ValueError("No transmittance columns found in " + csv_path)
...
class Q1ShiftedSpectraDataset(Dataset):
    def __init__(self, csv_file, max_points=4):
        ...
        self.df = pd.read_csv(csv_file, engine='python', on_bad_lines='skip')
        self.t_cols = [c for c in self.df.columns if c.startswith("T@")]
        if len(self.t_cols) == 0:
            raise ValueError("No transmittance columns found (T@...).")
...
class ShapeToSpectraModel(nn.Module):
    ...
    def forward(self, shape_4x3):
        ...
        # Apply sigmoid to bound the transmittance spectrum between 0 and 1
        out_2d = torch.sigmoid(out_flat.view(bsz, 11, 100))
        return out_2d
