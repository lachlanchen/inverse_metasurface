...
def train_with_random_noise(shape2filter_path, filter2shape_path, output_dir, min_snr=10, max_snr=40, 
    ...
    encoder_scheduler = optim.lr_scheduler.CosineAnnealingLR(
        encoder_optimizer, T_max=num_epochs, eta_min=0.0001
    )
    decoder_scheduler = optim.lr_scheduler.CosineAnnealingLR(
        decoder_optimizer, T_max=num_epochs, eta_min=0.00001
    )
    ...
    for epoch in range(num_epochs):
        ...
        # For CosineAnnealingLR or OneCycleLR
        encoder_scheduler.step()
        decoder_scheduler.step()
        # Optional: Print current learning rates
        current_encoder_lr = encoder_optimizer.param_groups[0]['lr']
        current_decoder_lr = decoder_optimizer.param_groups[0]['lr']
        print(f"Epoch {epoch}, Encoder LR: {current_encoder_lr:.6f}, Decoder LR: {current_decoder_lr:.6f}")
...
def train_with_fixed_shape(shape_name, shape, shape2filter_path, train_loader, test_loader, train_data, test_data,
    ...
    # Get sample batch for visualization (use fixed samples)
    # Choose first sample for consistent visualization
    ...
    train_sample = train_data[:1].to(device)
    ...
    for epoch in range(num_epochs):
        ...
        # Optional: Print current learning rates
        current_encoder_lr = encoder_optimizer.param_groups[0]['lr']
        print(f"Epoch {epoch}, Encoder LR: {current_encoder_lr:.6f}")
        ...
        train_mse, train_psnr, train_sam = calculate_metrics_in_batch(
            full_train_data, model, batch_size=batch_size, device=device,
            desc=f"Calculating {shape_name} train metrics", is_fixed_shape=True)

        # Calculate metrics on full test set
        test_mse, test_psnr, test_sam = calculate_metrics_in_batch(
            full_test_data, model, batch_size=batch_size, device=device,
            desc=f"Calculating {shape_name} test metrics", is_fixed_shape=True)
    ...
    model.decoder.load_state_dict(best_model_state)
    print(f"Loaded best model from epoch {best_epoch+1} with test MSE: {best_test_loss:.6f}")
...
def evaluate_fixed_shape_at_noise_level(model, shape_name, noise_level, train_data, test_data, 
    ...
    """
    Evaluate a trained fixed shape model at a specific noise level
    
    Parameters:
    model: Trained FixedShapeModel
    shape_name: Name of the shape for logs
    noise_level: SNR level in dB to evaluate at
    train_data: Training data tensor
    test_data: Testing data tensor
    batch_size: Batch size for evaluation
    output_dir: Directory to save results
    sample_indices: Indices of samples to use for visualization (tuple of train indices, test indices)
    ...
    # Update the model's noise level for fixed noise evaluation
    ...
    if sample_indices is None:
        # Use first, middle, and last sample
        if len(train_data) >= 3:
            train_viz_indices = [0, len(train_data) // 2, len(train_data) - 1]
        else:
            train_viz_indices = list(range(len(train_data)))
            
        if len(test_data) >= 3:
            test_viz_indices = [0, len(test_data) // 2, len(test_data) - 1]
        else:
            test_viz_indices = list(range(len(test_data)))
    else:
        train_viz_indices, test_viz_indices = sample_indices
    
    # Prepare visualization samples
    train_viz_samples = [train_data[i:i+1].to(device) for i in train_viz_indices]
    test_viz_samples = [test_data[i:i+1].to(device) for i in test_viz_indices]
    
    # Calculate metrics on full datasets with fixed noise level
    with torch.no_grad():
        # Calculate metrics on training set
        train_mse, train_psnr, train_sam = calculate_metrics_in_batch(
            train_data, model, batch_size=batch_size, device=device,
            desc=f"Evaluating {shape_name} on train data at {noise_level}dB",
            is_fixed_shape=True
        )
        
        # Calculate metrics on test set
        test_mse, test_psnr, test_sam = calculate_metrics_in_batch(
            test_data, model, batch_size=batch_size, device=device,
            desc=f"Evaluating {shape_name} on test data at {noise_level}dB",
            is_fixed_shape=True
        )
    ...
    # Print metrics
    print(f"\n{shape_name} evaluation at {noise_level} dB:")
    print(f"  Train - MSE: {train_mse:.6f}, PSNR: {train_psnr:.2f} dB, SAM: {train_sam:.6f} rad")
    print(f"  Test  - MSE: {test_mse:.6f}, PSNR: {test_psnr:.2f} dB, SAM: {test_sam:.6f} rad")
    print(f"  Filter condition number: {condition_number:.4f}")
    
    # Create visualizations for fixed samples
    # Training samples
    for i, train_sample in enumerate(train_viz_samples):
        ...
        with torch.no_grad():
            ...
            plt.subplot(1, 3, 1)
            im1 = plt.imshow(orig, cmap='viridis', vmin=vmin, vmax=vmax)
            plt.title(f'Original (Train Sample {idx})')
            plt.colorbar(im1, fraction=0.046, pad=0.04)
            
            # Plot reconstruction
            plt.subplot(1, 3, 2)
            im2 = plt.imshow(recon, cmap='viridis', vmin=vmin, vmax=vmax)
            plt.title(f'Reconstructed ({noise_level}dB)')
            plt.colorbar(im2, fraction=0.046, pad=0.04)
    ...
    for i, test_sample in enumerate(test_viz_samples):
        ...
        with torch.no_grad():
            ...
            # Select middle band for visualization
            band_idx = test_sample.shape[-1] // 2
            
            # Get data as numpy
            orig = test_sample[0].permute(2, 0, 1)[band_idx].cpu().numpy()
            recon = test_out[0].permute(2, 0, 1)[band_idx].cpu().numpy()
            diff = orig - recon
            ...
            plt.subplot(1, 3, 1)
            im1 = plt.imshow(orig, cmap='viridis', vmin=vmin, vmax=vmax)
            plt.title(f'Original (Test Sample {idx})')
            plt.colorbar(im1, fraction=0.046, pad=0.04)
    ...
    with open(os.path.join(eval_dir, 'evaluation_metrics.txt'), 'w') as f:
        ...
        f.write(f"  Train - MSE: {train_mse:.6f}, PSNR: {train_psnr:.2f} dB, SAM: {train_sam:.6f} rad\n")
        f.write(f"  Test  - MSE: {test_mse:.6f}, PSNR: {test_psnr:.2f} dB, SAM: {test_sam:.6f} rad\n")
    
    # Save metrics as NumPy arrays
    np.save(os.path.join(eval_dir, f'train_mse_{noise_level}dB.npy'), np.array([train_mse]))
    np.save(os.path.join(eval_dir, f'test_mse_{noise_level}dB.npy'), np.array([test_mse]))
    np.save(os.path.join(eval_dir, f'train_psnr_{noise_level}dB.npy'), np.array([train_psnr]))
    np.save(os.path.join(eval_dir, f'test_psnr_{noise_level}dB.npy'), np.array([test_psnr]))
    np.save(os.path.join(eval_dir, f'train_sam_{noise_level}dB.npy'), np.array([train_sam]))
    np.save(os.path.join(eval_dir, f'test_sam_{noise_level}dB.npy'), np.array([test_sam]))
...
def train_multiple_fixed_shapes(shapes_dict, shape2filter_path, output_dir, 
                               ...
                               viz_interval_stage2=1, use_s4=False):
    """
    ...
    # Dictionary to store trained models for each shape
    trained_models = {}
    
    # First, train a model for each shape with random noise
    # Use a default noise level of 25dB for training only (not important for evaluation)
    training_noise_level = 25.0
    print(f"\n{'='*50}")
    print(f"STEP 1: Training models for each shape with random noise at {training_noise_level}dB")
    print(f"{'='*50}")
    
    for shape_name, shape in shapes.items():
        print(f"\nTraining {shape_name} shape model with random noise")
        
        # Train the model
        model, metrics = train_with_fixed_shape(
            ...
            test_data=test_data,
            noise_level=training_noise_level,  # Base noise level (just for training)
            ...
            use_s4=use_s4  # Add this parameter
        )
        
        # Store the trained model
        trained_models[shape_name] = model
        
        # Calculate and store condition number
        with torch.no_grad():
            condition_number = calculate_condition_number(model.fixed_filter.detach().cpu())
            print(f"  Condition number for {shape_name}: {condition_number:.4f}")
    
    # Select fixed samples for visualization (use same samples across all evaluations)
    # Use first, middle, and last samples from train and test sets
    if len(train_data) >= 3:
        train_viz_indices = [0, len(train_data) // 2, len(train_data) - 1]
    else:
        train_viz_indices = list(range(len(train_data)))
    ...
    fixed_viz_indices = (train_viz_indices, test_viz_indices)
    
    # Now, evaluate each trained model at different fixed noise levels
    print(f"\n{'='*50}")
    print(f"STEP 2: Evaluating trained models at different fixed noise levels")
    print(f"{'='*50}")
    
    # Dictionary to store evaluation results for each shape and noise level
    eval_results = {
        'train_mse': {shape_name: [] for shape_name in shapes_dict},
        'test_mse': {shape_name: [] for shape_name in shapes_dict},
        'train_psnr': {shape_name: [] for shape_name in shapes_dict},
        'test_psnr': {shape_name: [] for shape_name in shapes_dict},
        'train_sam': {shape_name: [] for shape_name in shapes_dict},
        'test_sam': {shape_name: [] for shape_name in shapes_dict},
        'condition_number': {shape_name: None for shape_name in shapes_dict}
    }
    
    # For each noise level, evaluate all shapes
    for noise_level in noise_levels:
        print(f"\n{'='*50}")
        print(f"Evaluating at noise level: {noise_level} dB")
        print(f"{'='*50}")
        ...
        # Evaluate each shape at this noise level
        for shape_name, model in trained_models.items():
            print(f"\nEvaluating {shape_name} shape at noise level {noise_level} dB")
            
            # Evaluate the model at this noise level
            eval_metrics = evaluate_fixed_shape_at_noise_level(
                model=model,
                shape_name=shape_name,
                noise_level=noise_level,
                train_data=train_data,
                ...
                sample_indices=fixed_viz_indices
            )
            
            # Store metrics for this shape at this noise level
            eval_results['train_mse'][shape_name].append(eval_metrics['train_mse'])
            eval_results['test_mse'][shape_name].append(eval_metrics['test_mse'])
            eval_results['train_psnr'][shape_name].append(eval_metrics['train_psnr'])
            eval_results['test_psnr'][shape_name].append(eval_metrics['test_psnr'])
            eval_results['train_sam'][shape_name].append(eval_metrics['train_sam'])
            eval_results['test_sam'][shape_name].append(eval_metrics['test_sam'])
            
            # Store condition number (only needs to be stored once per shape)
            if eval_results['condition_number'][shape_name] is None:
                eval_results['condition_number'][shape_name] = eval_metrics['condition_number']
            
            # Save metrics for comparison at this noise level
            noise_metrics['train_mse'][shape_name] = eval_metrics['train_mse']
            noise_metrics['test_mse'][shape_name] = eval_metrics['test_mse']
            noise_metrics['train_psnr'][shape_name] = eval_metrics['train_psnr']
            noise_metrics['test_psnr'][shape_name] = eval_metrics['test_psnr']
            noise_metrics['train_sam'][shape_name] = eval_metrics['train_sam']
            noise_metrics['test_sam'][shape_name] = eval_metrics['test_sam']
            noise_metrics['condition_number'][shape_name] = eval_metrics['condition_number']
        ...
        plt.subplot(1, 2, 1)
        plt.bar(list(noise_metrics['train_mse'].keys()), 
                list(noise_metrics['train_mse'].values()), 
                color='skyblue', alpha=0.7)
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('MSE')
        plt.title(f'Training MSE at {noise_level} dB')
        
        plt.subplot(1, 2, 2)
        plt.bar(list(noise_metrics['test_mse'].keys()), 
                list(noise_metrics['test_mse'].values()), 
                color='lightcoral', alpha=0.7)
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('MSE')
        plt.title(f'Testing MSE at {noise_level} dB')
        ...
        plt.figure(figsize=(12, 6))
        plt.subplot(1, 2, 1)
        plt.bar(list(noise_metrics['train_psnr'].keys()), 
                list(noise_metrics['train_psnr'].values()), 
                color='skyblue', alpha=0.7)
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('PSNR (dB)')
        plt.title(f'Training PSNR at {noise_level} dB')
        
        plt.subplot(1, 2, 2)
        plt.bar(list(noise_metrics['test_psnr'].keys()), 
                list(noise_metrics['test_psnr'].values()), 
                color='lightcoral', alpha=0.7)
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('PSNR (dB)')
        plt.title(f'Testing PSNR at {noise_level} dB')
        
        plt.tight_layout()
        plt.savefig(os.path.join(noise_results_dir, 'psnr_comparison.png'))
        plt.close()
        
        # 3. SAM comparison bar chart
        plt.figure(figsize=(12, 6))
        plt.subplot(1, 2, 1)
        plt.bar(list(noise_metrics['train_sam'].keys()), 
                list(noise_metrics['train_sam'].values()), 
                color='skyblue', alpha=0.7)
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('SAM (radians)')
        plt.title(f'Training SAM at {noise_level} dB')
        
        plt.subplot(1, 2, 2)
        plt.bar(list(noise_metrics['test_sam'].keys()), 
                list(noise_metrics['test_sam'].values()), 
                color='lightcoral', alpha=0.7)
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('SAM (radians)')
        plt.title(f'Testing SAM at {noise_level} dB')
        ...
        plt.figure(figsize=(10, 6))
        plt.bar(list(noise_metrics['condition_number'].keys()), 
                list(noise_metrics['condition_number'].values()), 
                color='skyblue')
        plt.grid(True, axis='y')
        plt.xlabel('Shape Type')
        plt.ylabel('Condition Number')
        plt.title(f'Filter Condition Number Comparison')
        for i, (shape_name, cn) in enumerate(noise_metrics['condition_number'].items()):
        ...
        for shape_name in shapes:
            ...
            final_metrics['Train MSE'].append(noise_metrics['train_mse'][shape_name])
            final_metrics['Test MSE'].append(noise_metrics['test_mse'][shape_name])
            final_metrics['Train PSNR'].append(noise_metrics['train_psnr'][shape_name])
            final_metrics['Test PSNR'].append(noise_metrics['test_psnr'][shape_name])
            final_metrics['Train SAM'].append(noise_metrics['train_sam'][shape_name])
            final_metrics['Test SAM'].append(noise_metrics['test_sam'][shape_name])
            final_metrics['Condition Number'].append(noise_metrics['condition_number'][shape_name])
        
        # Create table and save as CSV
        with open(os.path.join(noise_results_dir, 'metrics_comparison.csv'), 'w') as f:
            # Write header
    ...
    plt.subplot(2, 1, 1)
    for shape_name in shapes:
        plt.plot(noise_levels, eval_results['train_mse'][shape_name], 'o-', 
                label=f"{shape_name.capitalize()}")
    plt.grid(True)
    plt.xlabel('Noise Level (SNR dB)')
    plt.ylabel('Train MSE')
    plt.title('Training MSE Across Noise Levels')
    plt.legend()
    
    plt.subplot(2, 1, 2)
    for shape_name in shapes:
        plt.plot(noise_levels, eval_results['test_mse'][shape_name], 'o-', 
                label=f"{shape_name.capitalize()}")
    plt.grid(True)
    plt.xlabel('Noise Level (SNR dB)')
    plt.ylabel('Test MSE')
    plt.title('Testing MSE Across Noise Levels')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'mse_across_noise.png'))
    plt.close()
    
    # 2. PSNR comparison across noise levels
    plt.figure(figsize=(12, 8))
    plt.subplot(2, 1, 1)
    for shape_name in shapes:
        plt.plot(noise_levels, eval_results['train_psnr'][shape_name], 'o-', 
                label=f"{shape_name.capitalize()}")
    plt.grid(True)
    plt.xlabel('Noise Level (SNR dB)')
    plt.ylabel('Train PSNR (dB)')
    plt.title('Training PSNR Across Noise Levels')
    plt.legend()
    
    plt.subplot(2, 1, 2)
    for shape_name in shapes:
        plt.plot(noise_levels, eval_results['test_psnr'][shape_name], 'o-', 
                label=f"{shape_name.capitalize()}")
    plt.grid(True)
    plt.xlabel('Noise Level (SNR dB)')
    plt.ylabel('Test PSNR (dB)')
    plt.title('Testing PSNR Across Noise Levels')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'psnr_across_noise.png'))
    plt.close()
    
    # 3. SAM comparison across noise levels
    plt.figure(figsize=(12, 8))
    plt.subplot(2, 1, 1)
    for shape_name in shapes:
        plt.plot(noise_levels, eval_results['train_sam'][shape_name], 'o-', 
                label=f"{shape_name.capitalize()}")
    plt.grid(True)
    plt.xlabel('Noise Level (SNR dB)')
    plt.ylabel('Train SAM (radians)')
    plt.title('Training SAM Across Noise Levels')
    plt.legend()
    
    plt.subplot(2, 1, 2)
    for shape_name in shapes:
        plt.plot(noise_levels, eval_results['test_sam'][shape_name], 'o-', 
                label=f"{shape_name.capitalize()}")
    plt.grid(True)
    plt.xlabel('Noise Level (SNR dB)')
    plt.ylabel('Test SAM (radians)')
    plt.title('Testing SAM Across Noise Levels')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'sam_across_noise.png'))
    plt.close()
    
    # Save numeric results
    for shape_name in shapes:
        np.save(os.path.join(results_dir, f'{shape_name}_train_mse.npy'), np.array(eval_results['train_mse'][shape_name]))
        np.save(os.path.join(results_dir, f'{shape_name}_test_mse.npy'), np.array(eval_results['test_mse'][shape_name]))
        np.save(os.path.join(results_dir, f'{shape_name}_train_psnr.npy'), np.array(eval_results['train_psnr'][shape_name]))
        np.save(os.path.join(results_dir, f'{shape_name}_test_psnr.npy'), np.array(eval_results['test_psnr'][shape_name]))
        np.save(os.path.join(results_dir, f'{shape_name}_train_sam.npy'), np.array(eval_results['train_sam'][shape_name]))
        np.save(os.path.join(results_dir, f'{shape_name}_test_sam.npy'), np.array(eval_results['test_sam'][shape_name]))
    ...
    with open(os.path.join(results_dir, 'summary.csv'), 'w') as f:
        # Write header
        f.write('Shape,Noise_Level,Train_MSE,Test_MSE,Train_PSNR,Test_PSNR,Train_SAM,Test_SAM,Condition_Number\n')
        
        # Write data
        for i, noise_level in enumerate(noise_levels):
            for shape_name in shapes:
                f.write(f"{shape_name},{noise_level},{eval_results['train_mse'][shape_name][i]:.6f},"
                        f"{eval_results['test_mse'][shape_name][i]:.6f},{eval_results['train_psnr'][shape_name][i]:.2f},"
                        f"{eval_results['test_psnr'][shape_name][i]:.2f},{eval_results['train_sam'][shape_name][i]:.6f},"
                        f"{eval_results['test_sam'][shape_name][i]:.6f},{eval_results['condition_number'][shape_name]:.4f}\n")
    
    # For backward compatibility with the original function interface
    # Copy evaluation metrics to results format expected by the original interface
    for shape_name in shapes:
        results['train_loss'][shape_name] = eval_results['train_mse'][shape_name]
        results['test_loss'][shape_name] = eval_results['test_mse'][shape_name]
        results['train_psnr'][shape_name] = eval_results['train_psnr'][shape_name]
        results['test_psnr'][shape_name] = eval_results['test_psnr'][shape_name]
        results['train_sam'][shape_name] = eval_results['train_sam'][shape_name]
        results['test_sam'][shape_name] = eval_results['test_sam'][shape_name]
    
    # Store the trained models in the results
