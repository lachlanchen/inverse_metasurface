...
###############################################################################
# DATASET FOR ORIGINAL CSV FILE (Fallback)
###############################################################################
...
def visualize_stageA_samples(model, ds_val, device, out_dir, sample_count=4, seed=123):
    ...
    with torch.no_grad():
        for i, idx_ in enumerate(idx_samples):
            ...
            # Right: Plot GT spec (blue) vs. predicted spec (red dashed).
            axR = axes[i][1]
...
def visualize_stageB_samples(pipeline, shape2spec_frozen, ds_val, device, out_dir, sample_count=4, seed=123):
    ...
    plt.savefig(out_fig)
    plt.close()
    print("[Stage B] Sample visualization saved to", out_fig)
...
def train_stageB_spec2shape_frozen(data_source, out_dir, shape2spec_ckpt, num_epochs=10,
...
def main():
    ...
    if not args.test:
        ...
        # Visualize Stage A results
        deviceA = next(modelA.parameters()).device
        visualize_stageA_samples(modelA, ds_valA, deviceA, stageA_dir)
        # Stage B training
        ...
        # Visualize Stage B results
        deviceB = next(spec2shape_net.parameters()).device
        pipeline = Spec2ShapeFrozen(spec2shape_net, shape2spec_froz)
        visualize_stageB_samples(pipeline, shape2spec_froz, ds_valB, deviceB, stageB_dir)
    else:
