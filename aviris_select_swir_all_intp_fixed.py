#!/usr/bin/env python3
"""
AVIRIS SWIR Band Selector with Interpolation (Fixed Version)
--------------------------------------------
Selects exactly 100 SWIR bands from AVIRIS data, removes bad bands through interpolation,
handles masked regions, and normalizes data to 0-1 range using global min/max values.
Specifically selects wavelengths in the 1000nm to 2.5μm range.

Features:
- Selects exactly 100 bands closest to the wavelengths in the CSV file
- Skips already processed files
- Uses chunked processing to avoid memory issues
- Creates detailed visualizations of results
- Rescales using global min/max values for consistent scaling
"""
...
def process_aviris_folder(base_folder, subfolder, csv_file, output_base, std_threshold=3.0, chunk_size=500, 
    ...
    if hasattr(img, 'bands') and hasattr(img.bands, 'centers'):
        ...
        # FIXED: Modified band selection to ensure we get exactly 100 bands
        print(f"\nSelecting exactly 100 bands closest to CSV wavelengths...")
        selected_bands = []
        wavelength_mapping = []
        
        # For each wavelength in the CSV, find the closest unique band in AVIRIS
        for i, wl_nm in enumerate(csv_wavelengths):
            # Find the closest matching band in AVIRIS data that hasn't been selected yet
            remaining_bands = [idx for idx in range(len(aviris_wavelengths)) if idx not in selected_bands]
            band_distances = np.abs(aviris_wavelengths[remaining_bands] - wl_nm)
            closest_idx = remaining_bands[np.argmin(band_distances)]
            ...
            wavelength_mapping.append({
                ...
                'AVIRIS_Band': closest_idx,
                'AVIRIS_Wavelength_nm': aviris_wavelengths[closest_idx],
                'AVIRIS_Wavelength_um': aviris_wavelengths[closest_idx]/1000,
                'Difference_nm': abs(wl_nm - aviris_wavelengths[closest_idx])
            })
        
        # Verify we have exactly 100 bands
        assert len(selected_bands) == 100, f"Expected 100 bands, but got {len(selected_bands)}"
        ...
        if extracted_data is None or extracted_data.shape[2] != len(selected_bands):
        ...
        if interpolated_data is None or interpolated_data.shape[2] != len(selected_bands):
        ...
        if cleaned_data is None or cleaned_data.shape[2] != len(selected_bands):
        ...
        if filled_data is None or filled_data.shape[2] != len(selected_bands):
        ...
        # FIXED: Calculate global min/max for rescaling
        print("\nFinding global min/max for rescaling...")
        global_data_min = np.inf
        global_data_max = -np.inf
        
        for i in tqdm(range(len(selected_bands)), desc="Computing global range"):
            if stats_df.iloc[i]['Is_Constant']:
                continue  # Skip constant bands
            ...
            global_data_min = min(global_data_min, band_min)
            global_data_max = max(global_data_max, band_max)
        
        print(f"\nGlobal data range for rescaling: Min={global_data_min:.6f}, Max={global_data_max:.6f}")
        
        # Check for rescaled data checkpoint or redo if we need to use global min/max
        rescaled_data = load_checkpoint(subfolder, "rescaled_data", output_base)
        
        if rescaled_data is None or rescaled_data.shape[2] != len(selected_bands):
            # FIXED: Rescale data to 0-1 range using global min/max
            rescaled_data = filled_data.copy()
            
            # Store original data for statistics
            original_cleaned_data = filled_data.copy()
            
            # Rescale all bands using global min/max
            print("\nRescaling data to 0-1 range using global min/max...")
            for i in tqdm(range(len(selected_bands)), desc="Rescaling data"):
                ...
                # FIXED: Rescale using global min/max
                rescaled_data[:,:,i] = (band_data - global_data_min) / (global_data_max - global_data_min)
        ...
        metadata = {
            ...
            'original_min': float(global_data_min),  # FIXED: Use global min/max
            'original_max': float(global_data_max),  # FIXED: Use global min/max
            ...
            'processing_steps': [
                'Extract 100 bands closest to CSV wavelengths',
                'Identify and interpolate bad bands',
                'Replace nodata values with band mean values',
                'Clean outliers using statistical approach',
                'Rescale all bands to 0-1 range using global min/max'
            ]
        }
