...
3) We create partial_crys_data/partial_crys_CX.csv for X in [0.0..1.0], step=0.1.
4) Now, we allow a command-line argument N (default=100) to decide how many
   uniformly spaced wavelengths we want in the final interpolation.

Usage:
  python partial_crys_mg.py [N]

Examples:
  python partial_crys_mg.py      # => uses N=100 by default
  python partial_crys_mg.py 200  # => uses N=200 for a finer grid
"""
...
# ---------------------------
# 0. Parse optional argument N
# ---------------------------
if len(sys.argv) > 1:
    try:
        N = int(sys.argv[1])
    except ValueError:
        print(f"Invalid argument {sys.argv[1]}, using default N=100.")
        N = 100
else:
    N = 100
...
# 1. Define Data
# ---------------------------
# (Same as your original definitions)
...
def compute_epsilon_eff(e_a, e_c, C):
    """
    e_eff = (1 + 2L)/(1 - L), 
    L = C*((e_c - 1)/(e_c + 2)) + (1-C)*((e_a - 1)/(e_a + 2)).
    """
    term_c = (e_c - 1)/(e_c + 2)
    term_a = (e_a - 1)/(e_a + 2)
...
# ---------------------------
# 4. Define a uniform wavelength grid of size N
# ---------------------------
all_w_min = min(wavelengths_crys.min(), wavelengths_amor.min())
all_w_max = max(wavelengths_crys.max(), wavelengths_amor.max())

# Build a uniform grid of N points
all_wavelengths = np.linspace(all_w_min, all_w_max, N)

# Interpolate n,k for each material onto this uniform grid
# For "crys" material:
k_crys_interp = np.interp(all_wavelengths, k_crys_wavelengths, k_crys)
n_crys_interp = np.interp(all_wavelengths, wavelengths_amor, n_crys)  # <-- note: check if this is correct
# Actually, you have "n_crys" defined for "wavelengths_amor"? 
#  If not, pick the correct array: you have n_crys, wavelengths_amor => that might mismatch. 
#  Possibly you want: np.interp(all_wavelengths, wavelengths_crys, n_crys). 
n_crys_interp = np.interp(all_wavelengths, wavelengths_crys, n_crys)  # if that matches your data
...
plt.subplot(1, 2, 1)
for C in crystallinities:
    cfile = f"partial_crys_data/partial_crys_C{C:.1f}.csv"
