...
------------------------
This script reads original AVIRIS hyperspectral data, masks out -9999 values,
downsamples the data spatially by a factor of 10, and creates histograms and
visualizations to analyze the true distribution of values.
"""
...
def mask_and_downsample_band(band_data, nodata_value=NODATA_VALUE, downsample_factor=DOWNSAMPLE_FACTOR):
    """
    Mask out NoData values and downsample the band data.
    
    Parameters:
    - band_data: 2D numpy array containing the band data
    - nodata_value: Value to be treated as NoData (default: -9999)
    - downsample_factor: Factor by which to downsample spatially (default: 10)
    
    Returns:
    - downsampled_band: Downsampled band with NoData values replaced by mean
    """
    ...
    if np.all(mask):
        print("Warning: Band contains only NoData values")
        # If all values are NoData, return zeros of appropriate size
        rows, cols = band_data.shape
        downsampled_rows = rows // downsample_factor
        downsampled_cols = cols // downsample_factor
        return np.zeros((downsampled_rows, downsampled_cols))
    
    # Replace NoData values with NaN temporarily for calculation
    band_data_masked = band_data.copy().astype(float)
    band_data_masked[mask] = np.nan
    
    # Calculate mean of valid values
    valid_mean = np.nanmean(band_data_masked)
    
    # Replace NoData values with the mean of valid values
    band_data_masked[mask] = valid_mean
    
    # Downsample by taking every nth pixel (simple method)
    downsampled_band = band_data_masked[::downsample_factor, ::downsample_factor]
    
    return downsampled_band

def select_random_pixels(img_shape, downsampled=True):
    """Select random pixels for spectrum analysis"""
    if downsampled:
        rows = img_shape[0] // DOWNSAMPLE_FACTOR
        cols = img_shape[1] // DOWNSAMPLE_FACTOR
    else:
    ...
    for i in range(NUM_RANDOM_PIXELS):
        ...
        # If positions were selected from downsampled version, scale back to original
        if downsampled:
            orig_r = r * DOWNSAMPLE_FACTOR
            orig_c = c * DOWNSAMPLE_FACTOR
            random_positions.append((orig_r, orig_c, f"pixel_{i+1}"))
        else:
...
def plot_wavelength_histograms(img, wavelengths, output_folder, dataset_name):
    """Create histograms for each wavelength band with masked NoData values and downsampling"""
    ...
    for band_idx in tqdm(range(bands)):
        ...
        # Mask NoData values and downsample
        processed_band = mask_and_downsample_band(band_data)
        
        # Flatten for histogram
        band_data_flat = processed_band.flatten()
...
def save_wavelength_frames(img, wavelengths, output_folder, dataset_name):
    """Save visualization frames for each wavelength band with masked NoData values and downsampling"""
    ...
    for band_idx in tqdm(range(bands)):
        ...
        # Determine scale based on data (exclude any remaining NaN values)
        valid_data = processed_band[np.isfinite(processed_band)]
        if len(valid_data) > 0:
        ...
        # Display frame
        im = plt.imshow(processed_band, cmap='viridis', vmin=vmin, vmax=vmax)
        ...
        if band_idx in summary_indices:
            summary_idx = np.where(summary_indices == band_idx)[0][0]
            if summary_idx < len(axes):
                im = axes[summary_idx].imshow(processed_band, cmap='viridis', vmin=vmin, vmax=vmax)
...
def plot_pixel_spectra(img, wavelengths, pixels, output_folder, dataset_name):
    """Plot spectra for randomly selected pixels with masked NoData values"""
