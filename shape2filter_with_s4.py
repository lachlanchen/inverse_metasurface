...
class Shape2FilterWithS4(nn.Module):
    ...
    """
    def __init__(self, mode='transmittance', max_workers=4):
    ...
    def forward(self, shape_4x3):
        ...
        for b in range(batch_size):
            ...
            if np.sum(valid_vertices) > 0:
                ...
                s4_spectra_list = self.run_s4_simulations(polygon_str, f"shape_{b}")
...
def main():
    ...
    if args.use_s4:
        ...
        shape2spec_model = Shape2FilterWithS4(mode=args.mode, max_workers=args.max_workers).to(device)
    else:
