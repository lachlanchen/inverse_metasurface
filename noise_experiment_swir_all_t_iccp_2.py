...
# Set up paper-style plotting without requiring specific fonts
plt.rcParams.update({
    'font.family': 'serif',  # Just use the system's default serif font
    'font.size': 11,
    'axes.labelsize': 11,
    'axes.titlesize': 12,
    'xtick.labelsize': 10,
...
# Function to save figure data for later use
def save_figure_data(data_dict, filename, description=""):
    """
    Save data used to generate figures for later use
    ...
    print(f"Figure data saved to: {os.path.abspath(filename)}")
...
def load_aviris_swir_data(swir_base_path="AVIRIS_SWIR", tile_size=128, cache_file=None, use_cache=False):
    """
...
def plot_shape_with_c4(shape, title, save_path, xlim=(-0.7, 0.7), ylim=(-0.7, 0.7)):
    """Plot shape with C4 symmetry replication using paper-style formatting"""
    ...
    plt.scatter(shape[presence, 1], shape[presence, 2], color='red', s=80, label='Control Points')
    ...
    if len(active_points) > 0:
        ...
        if len(sorted_points) >= 3:
            ...
            plt.fill(polygon[:, 0], polygon[:, 1], 'g', alpha=0.3)
        else:
            # Just plot the points
            plt.scatter(c4_points[:, 0], c4_points[:, 1], color='green', alpha=0.3)
    
    plt.title(title)
    plt.legend(loc='upper right', frameon=False)
    plt.axis('equal')
    
    # Add light gray coordinate axes without grid
    plt.axhline(0, color='lightgray', linestyle='-', linewidth=0.5)
    plt.axvline(0, color='lightgray', linestyle='-', linewidth=0.5)
    
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save shape data for later use
    shape_data = {
        'shape': shape,
        'active_points': active_points if len(active_points) > 0 else np.array([]),
        'c4_points': replicate_c4(active_points) if len(active_points) > 0 else np.array([]),
        'title': title,
        'xlim': xlim,
        'ylim': ylim
    }
    data_path = save_path.replace('.png', '_data.npy')
    save_figure_data(shape_data, data_path, f"Shape data for {title}")
...
def train_with_noise_level(model_path, output_dir, noise_level, batch_size=10, num_epochs=500, 
    ...
    Returns:
    tuple: (initial_shape_path, min_cn_shape_path, final_shape_path, output_dir)
    """
    ...
    data = load_aviris_swir_data(swir_base_path="AVIRIS_SWIR", tile_size=128, cache_file=cache_file, use_cache=use_cache)
    data = data.to(device)
    ...
    # For tracking the shape with minimum condition number
    min_condition_number = initial_condition_number
    min_cn_shape = initial_shape.copy()
    min_cn_raw_params = model.raw_params.clone().detach()
    min_cn_raw_shift = model.raw_shift.clone().detach()
    min_cn_epoch = 0
    min_cn_filters = initial_filters.clone()
    ...
    plt.savefig(initial_filters_path, bbox_inches='tight')
    plt.close()
    
    # Save initial filters data
    filter_data = {
        'filters': initial_filters.numpy(),
        'condition_number': initial_condition_number,
        'wavelength_indices': np.arange(100)
    }
    save_figure_data(filter_data, initial_filters_path.replace('.png', '_data.npy'), 
                    f"Initial filters data for SNR {noise_level} dB")
    ...
    for epoch in range(num_epochs):
        ...
        if current_condition_number < min_condition_number:
            min_condition_number = current_condition_number
            min_cn_epoch = epoch + 1
            # Save the shape parameters
            min_cn_raw_params = model.raw_params.clone().detach()
            min_cn_raw_shift = model.raw_shift.clone().detach()
            # Get the actual shape
            min_cn_shape = model.get_current_shape().detach().cpu().numpy()
            min_cn_filters = current_filters.clone()
            
            # Save the shape with minimum condition number
            min_cn_shape_path = f"{output_dir}/min_cn_shape_epoch_{min_cn_epoch}.png"
            plot_shape_with_c4(min_cn_shape, 
                           f"Min CN Shape (Epoch {min_cn_epoch}, CN: {min_condition_number:.2f})", 
                           min_cn_shape_path)
            print(f"New minimum condition number: {min_condition_number:.4f} at epoch {min_cn_epoch}")
    ...
    # Make sure we have the best shape saved separately
    best_cn_shape_path = f"{output_dir}/best_cn_shape.png"
    plot_shape_with_c4(min_cn_shape, 
                   f"Best CN Shape (Epoch {min_cn_epoch}, CN: {min_condition_number:.2f})", 
                   best_cn_shape_path)
    ...
    plt.savefig(final_filters_path, bbox_inches='tight')
    plt.close()
    
    # Save final filters data
    filter_data = {
        'filters': final_filters.numpy(),
        'condition_number': final_condition_number,
        'wavelength_indices': np.arange(100)
    }
    save_figure_data(filter_data, final_filters_path.replace('.png', '_data.npy'), 
                    f"Final filters data for SNR {noise_level} dB")
    
    # Also save min condition number filters
    plt.figure(figsize=(6, 4))
    for i in range(11):
        plt.plot(min_cn_filters.numpy()[i], label=f'Filter {i}' if i % 3 == 0 else None)
    plt.xlabel("Wavelength Index")
    plt.ylabel("Filter Value")
    plt.title(f"Best CN Filters (SNR: {noise_level} dB, CN: {min_condition_number:.2f})")
    plt.legend(frameon=False)
    min_cn_filters_path = f"{output_dir}/min_cn_filters.png"
    plt.savefig(min_cn_filters_path, bbox_inches='tight')
    plt.close()
    
    # Save min CN filters data
    filter_data = {
        'filters': min_cn_filters.numpy(),
        'condition_number': min_condition_number,
        'wavelength_indices': np.arange(100)
    }
    save_figure_data(filter_data, min_cn_filters_path.replace('.png', '_data.npy'), 
                    f"Best CN filters data for SNR {noise_level} dB")
    ...
    plt.title(f"Filter Matrix Condition Number (SNR: {noise_level} dB)")
    condition_plot_path = f"{output_dir}/condition_number.png"
    plt.savefig(condition_plot_path, bbox_inches='tight')
    plt.close()
    
    # Save condition number data
    cn_data = {
        'epochs': np.arange(num_epochs+1),
        'condition_numbers': np.array(condition_numbers),
        'min_cn_epoch': min_cn_epoch,
        'min_condition_number': min_condition_number
    }
    save_figure_data(cn_data, condition_plot_path.replace('.png', '_data.npy'), 
                   f"Condition number data for SNR {noise_level} dB")
    ...
    save_figure_data(loss_data, loss_plot_path.replace('.png', '_data.npy'), 
                   f"Training loss data for SNR {noise_level} dB")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE")
    plt.title(f"Reconstruction MSE (SNR: {noise_level} dB)")
    ...
    save_figure_data(mse_data, mse_plot_path.replace('.png', '_data.npy'), 
                   f"MSE data for SNR {noise_level} dB")
    ...
    # Save min condition number parameters
    min_cn_model_path = f"{output_dir}/min_cn_model_state.pt"
    torch.save({
        'raw_params': min_cn_raw_params.cpu(),
        'raw_shift': min_cn_raw_shift.cpu(),
        'min_cn_epoch': min_cn_epoch,
        'condition_number': min_condition_number
    }, min_cn_model_path)
    ...
    np.save(f"{output_dir}/min_cn_shape.npy", min_cn_shape)
    np.save(f"{output_dir}/final_shape.npy", final_shape)
    np.save(f"{output_dir}/initial_filters.npy", initial_filters.numpy())
    np.save(f"{output_dir}/min_cn_filters.npy", min_cn_filters.numpy())
    ...
    with open(f"{output_dir}/training_params.txt", "w") as f:
        ...
        # Save min condition number shape parameters
        f.write(f"Minimum condition number: {min_condition_number:.4f} at epoch {min_cn_epoch}\n")
        f.write("Min CN shape parameters:\n")
        f.write(f"raw_params:\n{min_cn_raw_params.cpu().numpy()}\n")
        f.write(f"raw_shift: {min_cn_raw_shift.item()}\n\n")
        ...
        f.write(f"Condition number improvement: {initial_condition_number - min_condition_number:.4f}\n\n")
    ...
    # Return paths to the initial, min CN, and final shapes for later training
    initial_shape_npy_path = f"{output_dir}/initial_shape.npy"
    min_cn_shape_npy_path = f"{output_dir}/min_cn_shape.npy"
    final_shape_npy_path = f"{output_dir}/final_shape.npy"
    return initial_shape_npy_path, min_cn_shape_npy_path, final_shape_npy_path, output_dir

# Function to train the decoder only with a fixed shape
def train_decoder_only(model_path, shape_path, output_dir, noise_level, shape_type_label="", num_epochs=100, 
    ...
    shape_type_label: Label for the shape type (initial, minCN, final)
    num_epochs: Number of training epochs
    batch_size: Batch size for training
    learning_rate: Learning rate for optimizer
    ...
    decoder_dir = os.path.join(output_dir, f"decoder_only_{shape_type_label}_noise_{noise_level}dB")
    ...
    print(f"Loaded {shape_type_label} shape from: {shape_path}")
    ...
    print(f"Training only decoder with fixed {shape_type_label} shape, SNR: {noise_level} dB")
    ...
    with torch.no_grad():
        ...
        initial_mse = ((initial_recon - sample_tensor) ** 2).mean().item()
        print(f"Initial MSE with fixed {shape_type_label} shape: {initial_mse:.6f}")
    ...
    print(f"Starting decoder-only training for {shape_type_label} shape, SNR: {noise_level} dB...")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title(f"Decoder-only Training Loss ({shape_type_label}, SNR: {noise_level} dB)")
    loss_plot_path = f"{decoder_dir}/training_loss.png"
    plt.savefig(loss_plot_path, bbox_inches='tight')
    plt.close()
    ...
    save_figure_data(loss_data, loss_plot_path.replace('.png', '_data.npy'), 
                   f"Training loss data for {shape_type_label} shape, SNR {noise_level} dB")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE")
    plt.title(f"Reconstruction MSE ({shape_type_label}, SNR: {noise_level} dB)")
    mse_plot_path = f"{decoder_dir}/mse_values.png"
    plt.savefig(mse_plot_path, bbox_inches='tight')
    plt.close()
    ...
    save_figure_data(mse_data, mse_plot_path.replace('.png', '_data.npy'), 
                   f"MSE data for {shape_type_label} shape, SNR {noise_level} dB")
    
    # Save log file
    with open(f"{decoder_dir}/training_params.txt", "w") as f:
        f.write(f"Shape type: {shape_type_label}\n")
    ...
    print(f"Decoder-only training completed for {shape_type_label} shape, SNR: {noise_level} dB")
    print(f"Results saved to: {decoder_dir}/")
    
    return final_mse

def plot_noise_comparison(noise_levels, initial_mse_values, min_cn_mse_values, final_mse_values, output_dir):
    """
    Plot the comparison of MSE values for initial, min CN, and final shapes across different noise levels
    """
    plt.figure(figsize=(7, 5))
    
    # Plot lines
    plt.plot(noise_levels, initial_mse_values, 'o-', color='#E41A1C', label='Initial Shape')
    plt.plot(noise_levels, min_cn_mse_values, 's-', color='#377EB8', label='Min CN Shape')
    plt.plot(noise_levels, final_mse_values, '^-', color='#4DAF4A', label='Final Shape')
    
    # Add labels
    plt.xlabel("Noise Level (SNR in dB)")
    plt.ylabel("MSE after Decoder-Only Training")
    plt.title("Shape Performance Across Noise Levels")
    ...
    results_data = {
        ...
        'initial_mse_values': np.array(initial_mse_values),
        'min_cn_mse_values': np.array(min_cn_mse_values),
        'final_mse_values': np.array(final_mse_values)
    }
    save_figure_data(results_data, comparison_path.replace('.png', '_data.npy'), 
                    "MSE comparison across noise levels and shape types")
...
def main():
    ...
    for noise_level in noise_levels:
        ...
        initial_shape_path, min_cn_shape_path, final_shape_path, output_dir = train_with_noise_level(
    ...
    for noise_level in noise_levels:
        ...
        # Train decoder with min CN shape
        print(f"\nTraining decoder with min CN shape, SNR: {noise_level} dB")
        min_cn_mse = train_decoder_only(
            model_path=model_path,
            shape_path=paths['min_cn'],
            output_dir=decoder_output_dir,
            noise_level=noise_level,
            shape_type_label="min_cn",
            num_epochs=args.decoder_epochs,
            batch_size=args.batch_size,
            learning_rate=args.learning_rate,
            cache_file=args.cache,
        ...
        min_cn_mse_values.append(min_cn_mse)
    ...
    plot_noise_comparison(
        ...
        min_cn_mse_values=min_cn_mse_values,
