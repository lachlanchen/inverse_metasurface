...
def load_aviris_swir_data(swir_path="AVIRIS_SWIR/torch/aviris_swir.pt", tile_size=100, cache_file=None, use_cache=False):
    ...
    Parameters:
    swir_path: Path to the AVIRIS_SWIR data in PyTorch format
    ...
    abs_path = os.path.abspath(swir_path)
    print(f"Looking for AVIRIS_SWIR data at: {abs_path}")
    
    if not os.path.exists(swir_path):
        # Try alternate paths
        base_dir = os.path.dirname(os.path.abspath(__file__))
        alternative_paths = [
            os.path.join(base_dir, swir_path),
            os.path.join(base_dir, "AVIRIS_SWIR", "torch", "aviris_swir.pt"),
            os.path.join("AVIRIS_SWIR", "torch", "aviris_swir.pt"),
            # Try the ENVI format as a fallback
            os.path.join(base_dir, "AVIRIS_SWIR", "AV320231008t173943_L2A_OE_main_98b13fff_RFL_ORT.hdr"),
            os.path.join("AVIRIS_SWIR", "AV320231008t173943_L2A_OE_main_98b13fff_RFL_ORT.hdr")
        ]
        ...
        for alt_path in alternative_paths:
            ...
            if os.path.exists(alt_path):
                swir_path = alt_path
                print(f"Found file at: {os.path.abspath(swir_path)}")
                break
        ...
        if not os.path.exists(swir_path):
            ...
            if os.path.exists("AVIRIS_SWIR"):
                print("\nListing contents of AVIRIS_SWIR directory:")
                print(os.listdir("AVIRIS_SWIR"))
                
                if os.path.exists("AVIRIS_SWIR/torch"):
                    print("\nListing contents of AVIRIS_SWIR/torch directory:")
                    print(os.listdir("AVIRIS_SWIR/torch"))
            
            raise FileNotFoundError(f"Could not find AVIRIS_SWIR data file. Tried paths: {[abs_path] + alternative_paths}")
    
    # Load the data
    print(f"Loading data from: {swir_path}")
    
    # Check if the path ends with .pt or .hdr to determine loading method
    if swir_path.endswith('.pt'):
        # Load PyTorch tensor
        data = torch.load(swir_path)
        print(f"Loaded PyTorch data of shape: {data.shape}")
    else:
        # Assume it's ENVI format and load with spectral library
        import spectral
        img = spectral.open_image(swir_path)
...
def main():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_dir = f"noise_experiment_swir_results_{timestamp}"
