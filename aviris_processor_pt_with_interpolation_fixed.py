...
def process_aviris_data(aviris_folder, subfolder, csv_file, output_folder, tile_size=128, overlap=0,
    ...
    # Make sure we select exactly the number of bands in the CSV
    num_bands_needed = len(csv_swir_wavelengths)
    print(f"Need to select exactly {num_bands_needed} bands")
    
    # For each wavelength in the filtered CSV, find the closest matching band in AVIRIS
    selected_bands = []
    wavelength_mapping = []
    
    # Strategy: For each CSV wavelength, find the closest AVIRIS wavelength
    # If we've already selected that AVIRIS band, find the next closest one
    for i, wl_nm in enumerate(csv_swir_wavelengths):
        # Calculate distances to all AVIRIS wavelengths
        distances = np.abs(aviris_wavelengths - wl_nm)
        
        # Sort distances to find closest matches
        sorted_indices = np.argsort(distances)
        
        # Find the closest band that hasn't been selected yet
        for idx in sorted_indices:
            if idx not in selected_bands:
                ...
                wavelength_mapping.append({
                    ...
                    'AVIRIS_Band': idx,
                    'AVIRIS_Wavelength_nm': aviris_wavelengths[idx],
                    'AVIRIS_Wavelength_um': aviris_wavelengths[idx]/1000,
                    'Difference_nm': distances[idx]
                })
                break
    
    # Create a DataFrame for better display
    mapping_df = pd.DataFrame(wavelength_mapping)
    print(f"Selected {len(selected_bands)} unique AVIRIS bands")
    print("First 5 wavelength mappings:")
    print(mapping_df.head(5).to_string(index=False))
    
    # Verify we got the right number of bands
    assert len(selected_bands) == num_bands_needed, f"Selected {len(selected_bands)} bands, needed {num_bands_needed}"
