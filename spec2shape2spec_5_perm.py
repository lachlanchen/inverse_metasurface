...
"""
spec2shape2spec_varlen_enhanced_perm_test.py

A spec->shape->spec pipeline with presence chaining for variable 1..4 vertices
and an enhanced training objective:

    total_loss = 
       MSE(shape_pred, shape_gt)
     + MSE(shape2spec(gt_shape), spectra_input)  # shape2spec on GT shape
     + MSE(spec_pred, spectra_input)             # chain recon

**Now includes a permutation-invariance test**:
  after training, we take random samples from the validation set, permute
  the 11 rows in each sample’s input, and verify the predicted shape is
  (nearly) unchanged.

Visualization (3 columns):
  - Left: GT spectrum (blue)
  - Middle: GT shape(green) vs predicted shape(red)
  - Right: Original(blue), recon-from-pred-shape(red dashed), recon-from-GT-shape(green dashed)
"""
...
class Spectra2ShapeVarLen(nn.Module):
    ...
    No positional embeddings => effectively order-invariant if you do mean pooling
    across the 11 tokens in the transformer.
    """
    ...
    def forward(self, spectra_11x100):
        ...
        # mean-pool => permutation-invariant aggregator
...
def test_permutation_invariance(
    ...
    with torch.no_grad():
        for row_i, idx_ in enumerate(idx_samples):
            ...
            # Evaluate difference in shape (raw MSE):
...
def train_spec2shape2spec_varlen(
    ...
    plt.title("Enhanced spec->shape->spec (varlen) + Perm Test")
    plt.legend()
    ...
    if val_len>0:
        ...
        # Optionally: test permutation invariance on a few samples
