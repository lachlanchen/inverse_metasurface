...
- Trains 100 epochs, then inference on a "realistic" random reflection
  that uses fewer key points and linear interpolation to produce N_WAVES=100.

Updated final polygon plotting to rotate the valid first-quadrant points
by 0°, 90°, 180°, 270°, yielding negative coords for other quadrants => angle-sort => fill.
"""
...
# 4) Inference + Plot
###############################################################################
def generate_realistic_spectrum(n_waves=100, num_keys=8):
    """
    Generate a "realistic" random reflection in [0,1] by:
      - picking `num_keys` points in x=linspace(0..1),
      - random y in [0..1] (or e.g. [0.1..0.9]),
      - linear interpolate to produce n_waves points.
    Returns a torch.tensor shape [n_waves].
    """
    key_x = np.linspace(0,1,num_keys)
    # For more variety, let's keep them in [0.0..1.0].
    # You could narrow e.g. [0.2..0.8] if you prefer.
    key_y = np.random.rand(num_keys)
    # final x
    full_x = np.linspace(0,1,n_waves)
    full_y = np.interp(full_x, key_x, key_y)
    return torch.from_numpy(full_y.astype(np.float32))

@torch.no_grad()
def replicate_c4(verts):
    """
    Given points [N,2] in the first quadrant, replicate them by rotating
    0°, 90°, 180°, 270°, possibly negative coords in other quadrants.
    Returns shape [4N,2].
    """
    out_list=[]
...
def test_inference():
    """
    Instead of a uniform random from [0.25..0.75], now generate a 'realistic' spectrum
    by picking ~8 key points in [0..1], random y in [0..1], then linear interpolation
    => shape [N_WAVES].
    """
    ...
    # Generate a "realistic" random reflection in [0..1]
    random_sp= generate_realistic_spectrum(n_waves=N_WAVES, num_keys=8).unsqueeze(0)  # [1,100]
    ...
    plt.figure()
    plt.plot(x_axis, random_sp.squeeze(0).numpy(), marker='o', label="InputSpec (Interp)")
