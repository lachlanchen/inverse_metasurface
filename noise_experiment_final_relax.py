...
def create_drastic_initial_shape():
    ...
    raw_params = torch.tensor([
        [2.0, 1.5, -0.8],    # First vertex always present (high value) with large radius
        [2.0, -1.2, 0.2],  # Second vertex likely absent with different radius if present
        [2.0, 0.8, 1.5],    # Third vertex likely present with medium-large radius
        [2.0, 2.0, 0]    # Fourth vertex likely absent with very large radius if present
    ], dtype=torch.float32)
...
def differentiable_legal_shape(raw_params, raw_shift):
    """
    Input:
      raw_params: a 4×3 tensor, each row: [raw_v_pres, raw_x, raw_y]
    ...
    # Apply straight-through threshold at 0.5:
    v_pres_bin = straight_through_threshold(v_pres_prob, thresh=0.5)
    
    # --- Coordinates Mapping ---
    # Map raw_x and raw_y (columns 1 and 2) via sigmoid then linearly to [0.05, 0.65]
    x = 0.05 + 0.6 * torch.sigmoid(raw_params[:, 1])
    y = 0.05 + 0.6 * torch.sigmoid(raw_params[:, 2])
    
    # Apply vertex presence to coordinates (zero out inactive vertices)
    x = x * v_pres_bin
    y = y * v_pres_bin
    
    # Stack x and y to form 4x2 coordinates tensor
    coordinates = torch.stack([x, y], dim=1)
    
    # Concatenate presence and coordinates to form final 4x3 shape tensor
    final_shape = torch.cat([v_pres_bin.unsqueeze(1), coordinates], dim=1)
