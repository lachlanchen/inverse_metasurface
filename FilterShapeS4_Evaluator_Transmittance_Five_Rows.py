...
def plot_sample(axes, col, sample_data, s4_spectra_list, fig=None):
    ...
    for i, c in enumerate(c_values):
        ax_gt_spec.plot(x_axis, gt_spectra[i], color=colors[i], label=f"c={c:.1f}")
    ...
    # Row 3: Network spectrum of GT shape
    ax_gt_net_spec = axes[2, col]
    for i, c in enumerate(c_values):
        ax_gt_net_spec.plot(x_axis, gt_spec_net_np[i], color=colors[i], label=f"c={c:.1f}")
    ...
    for i, (c, s4_spec) in enumerate(s4_spectra_list):
        if s4_spec is not None:
            ax_s4_spec.plot(x_axis, s4_spec, color=colors[i], label=f"c={c:.1f}")
    ...
    # Row 5: Network output spectrum of predicted shape - TRANSMITTANCE
    ax_pred_net_spec = axes[4, col]
    for i, c in enumerate(c_values):
        ax_pred_net_spec.plot(x_axis, pred_spec_net_np[i], color=colors[i], label=f"c={c:.1f}")
    ...
    # Add a common legend for all spectrum plots in the last row only
    if col == 0 and fig is not None:  # Only for the first column
