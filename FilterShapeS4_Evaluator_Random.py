...
def parse_args():
    ...
    parser.add_argument('--random_mode', action='store_true',
                        help='Use random inputs instead of real data')
    parser.add_argument('--random_method', type=str, default='all',
                        choices=['uniform', 'normal', 'sigmoid', 'shapes', 'all'],
                        help='Method for generating random inputs')
...
def generate_random_uniform():
    """Generate random uniform input in range [0,1]"""
    return np.random.uniform(0, 1, (11, 100))

def generate_random_normal():
    """Generate random normal input"""
    return np.random.normal(0, 1, (11, 100))

def generate_random_sigmoid():
    """Generate random normal input passed through sigmoid"""
    rand_normal = np.random.normal(0, 1, (11, 100))
    return 1 / (1 + np.exp(-rand_normal))  # sigmoid

def generate_random_shapes():
    """Generate random shapes directly"""
    # For four points, each with presence and x,y coordinates
    shape = np.zeros((4, 3))
    
    # First point always present
    shape[0, 0] = 1.0
    shape[0, 1:] = np.random.uniform(0.1, 0.6, 2)  # Random x,y in quadrant 1
    
    # Other points randomly present and positioned
    for i in range(1, 4):
        shape[i, 0] = np.random.choice([0, 1], p=[0.3, 0.7])  # 70% chance to be present
        if shape[i, 0] > 0:
            shape[i, 1:] = np.random.uniform(0.1, 0.6, 2)
    
    return shape

def get_random_sample_data(method, device, spec2shape_model, shape2spec_model):
    """Generate random input data based on specified method"""
    # Generate the random input based on the method
    if method == 'uniform':
        rand_input = generate_random_uniform()
        method_name = "Random Uniform [0,1]"
    elif method == 'normal':
        rand_input = generate_random_normal()
        method_name = "Random Normal"
    elif method == 'sigmoid':
        rand_input = generate_random_sigmoid()
        method_name = "Random Normal + Sigmoid"
    elif method == 'shapes':
        # For shapes, we'll bypass the spec2shape model
        random_shape = generate_random_shapes()
        
        # Convert to tensor for shape2spec
        shape_tensor = torch.tensor(random_shape, dtype=torch.float32).unsqueeze(0).to(device)
        
        with torch.no_grad():
            # Get network prediction for the random shape
            spec_net = shape2spec_model(shape_tensor)
        
        # Process the shape for visualization
        valid_points = random_shape[:, 0] > 0.5
        if np.sum(valid_points) > 0:
            shape_q1 = random_shape[valid_points, 1:3]
            shape_full = replicate_c4(shape_q1)
            shape_full = sort_points_by_angle(shape_full)
        else:
            shape_full = None
            shape_q1 = None
        
        # We set the "gt_spectra" to the network prediction for consistency
        return {
            'uid': f"random_shape_{np.random.randint(10000)}",
            'method': "Random Direct Shape",
            'input_type': 'shape',
            'gt_spectra': spec_net.squeeze(0).cpu().numpy(),
            'gt_spec_net_np': spec_net.squeeze(0).cpu().numpy(),
            'pred_spec_net_np': spec_net.squeeze(0).cpu().numpy(),
            'gt_full': shape_full,
            'gt_q1': shape_q1,
            'pred_full': shape_full,
            'pred_q1': shape_q1,
            'random_input': None
        }
    
    # For methods other than direct shapes
    method_name = method
    
    # Convert to tensor for model input
    rand_tensor = torch.tensor(rand_input, dtype=torch.float32).unsqueeze(0).to(device)  # (1,11,100)
    
    # Run through models
    with torch.no_grad():
        # Get shape from random input
        pred_shape = spec2shape_model(rand_tensor)  # (1,4,3)
        
        # Get network prediction for the shape
    ...
    return {
        'uid': f"random_{method}_{np.random.randint(10000)}",
        'method': method_name,
        'input_type': 'spectrum',
        'gt_spectra': rand_input,  # The random input is treated as "GT spectrum"
        'gt_spec_net_np': None,    # No GT shape, so no GT network spectrum
        'pred_spec_net_np': pred_spec_net_np,
        'gt_full': None,           # No GT shape
        'gt_q1': None,
        'pred_full': pred_full,
        'pred_q1': pred_q1,
        'random_input': rand_input
    }
...
class ShapeToSpectraModel(nn.Module):
    ...
    def forward(self, shape_4x3):
        ...
        out_2d = torch.sigmoid(out_2d)  # Apply sigmoid to fix the model output
        return out_2d
...
def plot_sample(axes, col, sample_data, s4_spectra_list, fig=None):
    ...
    uid = sample_data['uid']
    input_type = sample_data['input_type']
    ...
    uid_short = uid.split('_')[-1] if '_' in uid else uid
    title = f"{method}\nSample {uid_short}"
    ax_shape.set_title(title, fontsize=12, fontweight='bold')
    ...
    if gt_full is not None or pred_full is not None:
    ...
    # Row 2: Input spectrum (either GT or random) - TRANSMITTANCE
    ...
    input_title = "GT Shape → S4 Spectra" if input_type == 'spectrum' else "Random Input"
    ax_gt_spec.set_title(input_title, fontsize=11)
    ax_gt_spec.set_xlabel("Wavelength Index", fontsize=10)
    ax_gt_spec.set_ylabel("Value", fontsize=10)
    ax_gt_spec.grid(True, linestyle='--', alpha=0.3)
    if input_type == 'spectrum':
        ax_gt_spec.set_ylim(0, 1)
    
    # Row 3: Network spectrum of GT shape (if available) - using inferno colormap
    ax_gt_net_spec = axes[2, col]
    if gt_spec_net_np is not None:
        for i, c in enumerate(c_values):
        ...
        # For random input, this plot is not applicable
        ax_gt_net_spec.set_title("N/A", fontsize=11)
        ax_gt_net_spec.axis('off')
    ...
    if s4_spectra_list:
        ...
        ax_s4_spec.set_title("No S4 Spectra Available", fontsize=11)
    ...
    if pred_spec_net_np is not None:
        for i, c in enumerate(c_values):
        ...
        ax_pred_net_spec.set_title("No Network Spectra Available", fontsize=11)
    ...
    if col == 3 and fig is not None:  # Only for the last column
        # Set up two different legends for the two colormaps
        viridis_handles, viridis_labels = ax_gt_spec.get_legend_handles_labels()
        inferno_handles, inferno_labels = ax_pred_net_spec.get_legend_handles_labels()
        
        # Create the viridis legend
        viridis_legend = fig.legend(
            viridis_handles, viridis_labels, 
            loc='center right', 
            bbox_to_anchor=(1.12, 0.7), 
            title="Viridis (S4 Spectra)",
            fontsize=9
        )
        viridis_legend.get_frame().set_alpha(0.9)
        viridis_legend.get_frame().set_edgecolor('lightgray')
        
        # Create the inferno legend
        inferno_legend = fig.legend(
            inferno_handles, inferno_labels, 
            loc='center right', 
            bbox_to_anchor=(1.12, 0.3), 
            title="Inferno (Network)",
            fontsize=9
        )
        inferno_legend.get_frame().set_alpha(0.9)
        inferno_legend.get_frame().set_edgecolor('lightgray')
...
def main():
    ...
    if args.out_folder:
        ...
        folder_suffix = "Random" if args.random_mode else "Standard"
        out_folder = f"FilterShapeS4_Evaluator_{folder_suffix}_{dt_str}"
    ...
    if args.random_mode:
        # Process random input mode
        print(f"[INFO] Running in random input mode with method: {args.random_method}")
        
        if args.random_method == 'all':
            # Process all random methods
            methods = ['uniform', 'normal', 'sigmoid', 'shapes']
        else:
            # Process the specified method
            methods = [args.random_method]
        
        # Generate sample data for each method
        samples = []
        for method in methods:
            for i in range(1):  # Just one sample per method for now
                sample_data = get_random_sample_data(method, device, spec2shape_model, shape2spec_model)
                samples.append(sample_data)
        
        # Create groups of up to 4 samples each
        groups = [samples[i:i+4] for i in range(0, len(samples), 4)]
    else:
        # Standard mode with real data
    ...
    # For each group, create a 5-row x 4-column figure
    for g_idx, group in enumerate(groups):
        # Create figure with better proportions for publication
        fig, axes = plt.subplots(5, 4, figsize=(16, 16), constrained_layout=False)
        ...
        for col in range(4):
            ...
            if args.random_mode:
                # In random mode, we already have the sample data
                sample_data = group[col]
                print(f"[INFO] Group {g_idx+1} - Processing random sample: {sample_data['method']}")
            else:
                # In standard mode, process the data from the dataset
                idx = group[col]
                uid = uids[idx]
            ...
            if sample_data['pred_full'] is not None:
                ...
                s4_spectra_list = run_s4_simulations(polygon_str, sample_data['uid'], args.max_workers)
            else:
                print(f"[ERROR] Predicted shape is empty for UID {sample_data['uid']}. Skipping S4 simulation.")
        ...
        plt.savefig(outpng_hires, dpi=300, bbox_inches='tight')
        
        outpdf = os.path.join(out_folder, f"figure_group_{g_idx+1}.pdf")
        plt.savefig(outpdf, format='pdf', bbox_inches='tight')
