...
# Function to load AVIRIS_SWIR_INTP data from all available subfolders
def load_aviris_swir_data(swir_base_path="AVIRIS_SWIR_INTP", tile_size=128, cache_file=None, use_cache=False):
    """
    Load pre-processed AVIRIS_SWIR_INTP hyperspectral data from all available subfolders and crop it into tiles
...
def train_with_noise_level(model_path, output_dir, noise_level, batch_size=10, num_epochs=500, 
    ...
    data = load_aviris_swir_data(swir_base_path="AVIRIS_SWIR_INTP", tile_size=128, cache_file=cache_file, use_cache=use_cache)
    data = data.to(device)
