...
- Updated final polygon plotting to angle-sort the C4-rotated points, close them, and fill
"""
...
class ForcedFirstVertexModel(nn.Module):
    ...
    def model(self, spectrum, c_vals, is_c_known, v_pres, v_where, is_v_known):
        ...
        with pyro.plate("data", B):
            ...
            # location t=0 => raw => Normal(0,1) => softplus
...
def test_inference():
    ...
    random_sp= (torch.rand(N_WAVES)*0.5+0.25).unsqueeze(0) # shape [1,100]
    ...
    plt.figure()
    plt.plot(x_axis, random_sp.squeeze(0).numpy(), marker='o', label="InputSpec")
    ...
    # gather present vertices in quadrant
    keep_pts=[]
    # forced first
