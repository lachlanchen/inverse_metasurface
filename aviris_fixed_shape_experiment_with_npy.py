...
def plot_stage2_comparison(stage2_results, args):
    ...
    # Save the stage2_results as numpy files
    for data_type in ['train_losses', 'test_losses', 'condition_numbers']:
        if data_type in stage2_results and stage2_results[data_type]:
            # Convert to numpy arrays for better compatibility
            data_dict = {}
            for shape_name, data in stage2_results[data_type].items():
                if isinstance(data, list):
                    data_dict[shape_name] = np.array(data)
                else:
                    data_dict[shape_name] = data
            
            # Save as .npy file
            np.save(os.path.join(plots_dir, f"{data_type}.npy"), data_dict)
            print(f"Saved {data_type} data to {os.path.join(plots_dir, f'{data_type}.npy')}")
