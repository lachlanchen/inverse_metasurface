...
--
-- Modified for NIR region with specific thickness, lattice size, and radius range.
--
...
local default_base_outer  = 0.225  -- Changed to 0.225 for range 0.05 to 0.4
local default_rand_outer  = 0.175  -- Changed to 0.175 for range 0.05 to 0.4
...
-- Example usage:
--   ../build/S4 -a "myPrefix 99999 120 0.225 0.175 2 50000 -s -v" metasurface_resume_random_state_nir.lua
...
------------------------------------------------------------------------------
-- 1) CSV listing (all gsst_partial_crys_data/*.csv)
------------------------------------------------------------------------------
...
local folder = "gsst_partial_crys_data"
...
local final_name = string.format(
    "%s_nir_seed%d_g%d_nQ%d_nS%d_b%.2f_r%.2f",
    prefix, random_seed, num_g, N_quarter, num_shapes, base_outer, rand_outer
)

local shapes_subfolder = "shapes-nir/"..final_name
local out_filename     = "results-nir/"..final_name..".csv"
...
for shape_idx = resume_shape_idx, num_shapes do
    ...
    for csv_i = start_csv_index, #all_csvs do
        ...
        local matname = "GSST_C"
        do
            local c_str = csvfile:match("gsst_partial_crys_C([^/]+)%.csv")
            if c_str then
                matname = "GSST_C"..c_str
            end
        end
        ...
        for i = start_row, #data do
            ...
            -- Convert wavelength from nm to um for S4
            local lam_um = lam / 1000
            local freq = 1.0 / lam_um

            local eps_real = n*n - k*k
            local eps_imag = 2*n*k

            local S = S4.NewSimulation()
            S:SetLattice({0.6,0}, {0,0.6})  -- Changed to 0.6 um lattice size
            S:SetNumG(num_g)
            ...
            S:AddLayer("MetaLayer",  0.1, "Vacuum")  -- Changed to 0.1 um (100 nm) thickness
            ...
            if verbose then
                print(string.format(
                    ...
                    csvfile, shape_idx, i, lam_um, freq, n, k, R, T, (R+T)
                ))
            end
...
--------------------------------------------------------------------------------
-- End of metasurface_resume_random_state_nir.lua
--------------------------------------------------------------------------------
