...
Then we define 11 rotation angles and 11 c values. For each combination
(angle, c), we run S4 and store Reflection and Transmission vs wavelength.

Finally, we plot an 11×3 grid:
  - 11 rows (one per c value)
  - 3 columns (R, T, R+T)
In each subplot, we overlay 11 lines (angles),
but the legend only shows labels for angle = 0°, angle = mid°, angle = max°.
"""
...
import seaborn as sns  # for a nicer style
...
def generate_c4_symmetric_polygon(nQ=3, rmin=0.05, rmax=0.65):
    ...
    """
    # The random seed was set externally (see main()) for reproducibility.
...
# 2) S4 Run / Parsing
###############################################################################
def run_s4_for_polygon_and_c(poly_str, c_val,
...
###############################################################################
def main():
    # Use a nice Seaborn style & bigger fonts
    ...
    # 3) Angles in [0, pi/(2*nQ)] => 11 steps
    num_angles = 11
    ...
    print(f"[INFO] angle range = [0, {angle_max:.4f}] with {num_angles} steps")
    ...
    # 6) We'll do an 11 x 3 figure => 11 rows for c, 3 columns for R/T/R+T
    #    Each subplot has 11 lines for angles.
    #    But we only label angle 0, angle mid (num_angles//2), angle last in the legend.

    # Re-map the data so row = c
    ...
    angle_cmap = plt.cm.get_cmap("tab10", num_angles)
    # Indices we will label in the legend
    legend_label_indices = [0, num_angles // 2, num_angles - 1]
    ...
    for row_i, c_val in enumerate(c_values):
        ...
        for i, theta in enumerate(angle_list):
            ...
            color = angle_cmap(i)
            # Only show label for i in [0, mid, last]
            if i in legend_label_indices:
        ...
        # For the first row, place the legend
        if row_i == 0:
            ax_R.legend(loc="best", fontsize="small", title="Angle")
            ax_T.legend(loc="best", fontsize="small", title="Angle")
            ax_RT.legend(loc="best", fontsize="small", title="Angle")
