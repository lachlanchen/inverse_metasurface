...
class HyperspectralAutoencoder(nn.Module):
    def __init__(self, shape2spec_model_path):
        ...
        self.raw_params = nn.Parameter(torch.tensor([
            ...
            [0.0, 0.0],  # 50/50 chance for second vertex
            [0.0, 0.0],  # 50/50 chance for third vertex
            [0.0, 0.0]   # 50/50 chance for fourth vertex
        ], dtype=torch.float32))
...
def train_and_visualize_autoencoder(model_path, output_dir, batch_size=10, num_epochs=100, 
                              learning_rate=0.001, freeze_decoder=False):
    """
    ...
    if freeze_decoder:
        ...
        print("Optimizing both shape parameters and decoder")
    ...
    # Set learning rate higher for shape parameters if decoder is frozen
    if freeze_decoder:
        # Larger learning rate when optimizing only shape parameters
        learning_rate = 0.005
    
    # Set up optimizer - will automatically optimize only parameters with requires_grad=True
...
if __name__ == "__main__":
    ...
    # Change freeze_decoder to True to optimize only the shape parameters
    model, losses, output_dir = train_and_visualize_autoencoder(
        ...
        num_epochs=50,
        learning_rate=0.001,
        freeze_decoder=True  # Set to True to freeze the decoder
    )
