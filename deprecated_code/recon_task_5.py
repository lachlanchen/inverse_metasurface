...
def straight_through_threshold(x, thresh=0.5, temp=1.0):
    """
    ...
    Added temperature parameter to soften decision boundary.
    """
    # Softer sigmoid with temperature for smoother transitions
    soft_x = torch.sigmoid((x - thresh) * temp)
    hard_x = (x >= thresh).float()
    return hard_x + soft_x - soft_x.detach()

def differentiable_legal_shape(raw_params, raw_shift, temp=10.0):
    """
    Input:
      raw_params: a 4×2 tensor, each row: [raw_v_pres, raw_radius]
      raw_shift: a scalar tensor representing the raw angle shift.
      temp: temperature parameter for sigmoid steepness
    ...
    # Apply straight-through threshold at 0.5 with temperature:
    v_pres_bin = straight_through_threshold(v_pres_cum, thresh=0.5, temp=temp)
...
def optimize_shape_for_target_spectrum():
    ...
    # Freeze all network parameters - this is critical!
    for param in model.parameters():
        param.requires_grad = False
    print("Network parameters frozen successfully.")
    ...
    # Step 3: Setup optimization with improved stability
    
    # Use a smaller learning rate for more stability
    learning_rate = 0.005  # Reduced from 0.01
    
    # Add weight decay to stabilize parameters
    weight_decay = 1e-4
    
    # Use AdamW with weight decay instead of Adam
    optimizer = torch.optim.AdamW([
        {'params': opt_raw_params, 'lr': learning_rate, 'weight_decay': weight_decay},
        {'params': opt_raw_shift, 'lr': learning_rate * 0.5, 'weight_decay': weight_decay}
    ])
    
    # Create a learning rate scheduler that reduces LR over time
    ...
    # Temperature annealing for softening thresholds
    initial_temp = 5.0
    final_temp = 20.0
    ...
    for i in range(n_iterations):
        ...
        progress = min(i / (n_iterations * 0.7), 1.0)  # Reach final temp at 70% of iterations
        current_temp = initial_temp + progress * (final_temp - initial_temp)
        
        # Generate the shape from optimizable parameters with current temperature
        current_shape = differentiable_legal_shape(opt_raw_params, opt_raw_shift, temp=current_temp)
        ...
        # Add a small stability regularization term
        stability_reg = 0.001 * (torch.sum(torch.abs(opt_raw_params[:, 0])) + torch.abs(opt_raw_shift))
        total_loss = loss + stability_reg
        ...
        torch.nn.utils.clip_grad_norm_([opt_raw_params, opt_raw_shift], grad_clip_value)
        ...
        # Print progress
        if (i+1) % 20 == 0 or i == 0:
            print(f"Iteration {i+1}/{n_iterations}, Loss: {loss.item():.6f}, Temp: {current_temp:.2f}")
        ...
        if loss.item() < best_loss:
            ...
            best_params = opt_raw_params.clone().detach()
            best_shift = opt_raw_shift.clone().detach()
        else:
            patience_counter += 1
            
        if patience_counter >= patience:
            print(f"\nEarly stopping at iteration {i+1} as no improvement for {patience} iterations")
            break
    
    # Use the best parameters found
    if best_params is not None:
        # Final result using best parameters instead of last parameters
        with torch.no_grad():
            final_shape = differentiable_legal_shape(best_params, best_shift, temp=final_temp)
            final_spectrum = model(final_shape.unsqueeze(0))[0]
    else:
        # Use the last parameters if no better ones were found
        with torch.no_grad():
            final_shape = differentiable_legal_shape(opt_raw_params, opt_raw_shift, temp=final_temp)
            final_spectrum = model(final_shape.unsqueeze(0))[0]
