...
class Spec2ShapeFrozen(nn.Module):
    def __init__(self, spec2shape_net, shape2spec_frozen, no_grad_frozen=True):
        """
        no_grad_frozen: If True, then the fixed shape2spec network is computed
        inside a torch.no_grad() block, which means its output will be detached.
        For Stage C training (to allow gradient flow from the loss to the
        spec2shape network), set this to False.
        """
        super().__init__()
...
###############################################################################
# HELPER: print_model_params
###############################################################################
def print_model_params(model, model_name="model"):
    print("Parameters for {}:".format(model_name))
    for idx, (name, param) in enumerate(model.named_parameters()):
        print("{}: requires_grad={}, mean={:.6f}, sum={:.6f}".format(
            name, param.requires_grad, param.data.mean().item(), param.data.sum().item()
        ))
...
def visualize_stageB_gt_samples(spec2shape_net, ds_val, device, out_dir, sample_count=4, seed=123):
    ...
    for i in idx_samples:
        _, shape_gt, uid_ = ds_val[i]
        spec_gt, _, _ = ds_val[i]
...
def train_stageC_spec2shape_chain(data_source, out_dir, shape2spec_ckpt, spec2shape_ckpt, num_epochs=10,
    ...
    # In Stage C, we want the fixed shape2spec branch to be unchanged.
    # So we instantiate the pipeline with no_grad_frozen=False (to allow gradient flow to spec2shape)
...
def main():
    ...
    if not args.test:
        ...
        print_model_params(modelA, "Shape2Spec (Stage A) AFTER training")
        ...
        # Stage C training (spec->shape fine-tuned via chain loss, with fixed shape2spec)
        ...
        # Print the parameters of the fixed shape2spec branch BEFORE using it in Stage C
        print_model_params(shape2spec_frozen_C, "Fixed Shape2Spec (Stage C) BEFORE Stage C training")
        # IMPORTANT: In Stage C training, we want the shape2spec branch to be fixed.
        # To allow gradients to flow to spec2shape_net, we instantiate the pipeline with no_grad_frozen=False.
        pipeline_C = Spec2ShapeFrozen(spec2shape_net_C, shape2spec_frozen_C, no_grad_frozen=False)
        visualize_stageC_samples(pipeline_C, shape2spec_frozen_C, ds_valC, next(spec2shape_net_C.parameters()).device, stageC_dir)
        # After Stage C, print the parameters of the fixed shape2spec branch to ensure they did not change.
        print_model_params(shape2spec_frozen_C, "Fixed Shape2Spec (Stage C) AFTER Stage C training")
    else:
