...
def main():
    ...
    # Fixed c value for S4 (for visualization of one S4 spectrum)
    fixed_c = 0.5
    ...
    for g_idx, group in enumerate(groups):
        ...
        # If a column is not used (group has < 4 samples), we hide those subplots.
        for col in range(4):
            if col >= len(group):
                # Turn off all subplots in this column
                for row in range(4):
            ...
            # Run S4 for fixed c value on the predicted shape
            if pred_full is not None:
                polygon_str = polygon_to_string(pred_full)
                s4_csv = run_s4_for_c(polygon_str, fixed_c)
            ...
            # Row 4: S4 spectrum (using fixed c value)
            ax_s4_spec = axes[3, col]
            if s4_spec is not None:
                ax_s4_spec.plot(x_axis, s4_spec, 'm-', linewidth=2, label=f"S4 (c={fixed_c:.2f})")
            else:
                ax_s4_spec.text(0.5, 0.5, "S4 failed", transform=ax_s4_spec.transAxes, color='red')
            ax_s4_spec.set_title("S4 Spectrum")
