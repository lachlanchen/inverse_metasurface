...
class HyperspectralAutoencoder(nn.Module):
    def __init__(self, shape2filter_path, filter2shape_path, target_snr=None, random_init=True):
        super().__init__()
        ...
        if random_init:
            ...
            # Initialize to a more balanced filter (0.5)
            self.filter_params = nn.Parameter(torch.ones(11, 100, device=self.device) * 0.5)
    ...
    def get_current_filter(self):
        ...
        # return torch.sigmoid(self.filter_params)  # Ensure filter is in [0, 1]
...
def train_with_noise_level(shape2filter_path, filter2shape_path, output_dir, noise_level, 
    ...
    model = HyperspectralAutoencoder(shape2filter_path, filter2shape_path, target_snr=noise_level).to(device)
    ...
    for model_path in [shape2filter_path, filter2shape_path]:
        if not os.path.exists(model_path):
            ...
            for root, dirs, files in os.walk('.'):
                for file in files:
                    if file.endswith('.pt'):
                        if "shape2filter" in file and "shape2filter_path" in locals():
                            shape2filter_path = os.path.join(root, file)
                            print(f"Using shape2filter model file: {shape2filter_path}")
                        elif "filter2shape" in file and "filter2shape_path" in locals():
