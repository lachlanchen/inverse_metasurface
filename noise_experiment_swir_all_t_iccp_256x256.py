...
# Function to save hyperspectral reconstruction visualizations
def save_hyperspectral_visualizations(sample_np, recon_np, save_dir, prefix):
    """
    Save visualizations of hyperspectral sample and reconstruction
    
    Parameters:
    sample_np: Numpy array of shape [height, width, bands] - original sample
    recon_np: Numpy array of shape [height, width, bands] - reconstructed sample
    ...
    error_np = np.abs(sample_np - recon_np)
    
    # 1. Save RGB visualization (using bands 30, 20, 10 as R, G, B)
    plt.figure(figsize=(15, 5))
    
    # Original
    plt.subplot(1, 3, 1)
    rgb_original = np.stack([sample_np[:, :, 30], sample_np[:, :, 20], sample_np[:, :, 10]], axis=-1)
    ...
    # Reconstruction
    plt.subplot(1, 3, 2)
    rgb_recon = np.stack([recon_np[:, :, 30], recon_np[:, :, 20], recon_np[:, :, 10]], axis=-1)
    plt.imshow(np.clip(rgb_recon, 0, 1))
    plt.title("Reconstruction")
    plt.axis('off')
    
    # Error
    plt.subplot(1, 3, 3)
    plt.imshow(np.mean(error_np, axis=2), cmap='hot')
    plt.colorbar(fraction=0.046, pad=0.04)
    plt.title("Error")
    plt.axis('off')
    
    plt.tight_layout()
    plt.savefig(f"{save_dir}/{prefix}_rgb_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2. Save spectral profiles for a center pixel
    center_y, center_x = sample_np.shape[0] // 2, sample_np.shape[1] // 2
    
    plt.figure(figsize=(10, 5))
    plt.plot(sample_np[center_y, center_x, :], 'b-', label='Original')
    plt.plot(recon_np[center_y, center_x, :], 'r-', label='Reconstruction')
    plt.grid(True)
    plt.xlabel("Spectral Band")
    plt.ylabel("Value")
    plt.title(f"Spectral Profile at Center Pixel ({center_x}, {center_y})")
    plt.legend()
    plt.savefig(f"{save_dir}/{prefix}_spectral_profile.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3. Save mean spectral profiles across the entire image
    plt.figure(figsize=(10, 5))
    plt.plot(np.mean(sample_np, axis=(0, 1)), 'b-', label='Original')
    plt.plot(np.mean(recon_np, axis=(0, 1)), 'r-', label='Reconstruction')
    plt.grid(True)
    plt.xlabel("Spectral Band")
    plt.ylabel("Mean Value")
    plt.title("Mean Spectral Profile")
    plt.legend()
    plt.savefig(f"{save_dir}/{prefix}_mean_spectral_profile.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # 4. Save the data
    np.save(f"{save_dir}/{prefix}_sample.npy", sample_np)
    np.save(f"{save_dir}/{prefix}_recon.npy", recon_np)
    np.save(f"{save_dir}/{prefix}_error.npy", error_np)
    
    # 5. Save a error distribution histogram
    plt.figure(figsize=(10, 5))
    plt.hist(error_np.flatten(), bins=100, alpha=0.7)
    plt.grid(True)
    plt.xlabel("Absolute Error")
    plt.ylabel("Frequency")
    plt.title("Error Distribution")
    plt.savefig(f"{save_dir}/{prefix}_error_distribution.png", dpi=300, bbox_inches='tight')
    plt.close()

# Function to load AVIRIS_SWIR data from all available subfolders
def load_aviris_swir_data(swir_base_path="AVIRIS_SWIR", tile_size=256, cache_file=None, use_cache=False):
    """
    ...
    tile_size: Size of the tiles (square) - changed from 100 to 256
...
def train_with_noise_level(model_path, output_dir, noise_level, batch_size=10, num_epochs=500, 
    ...
    recon_dir = os.path.join(output_dir, "reconstruction_viz")
    ...
    # Load AVIRIS_SWIR data with caching support - now using 256x256 tiles
    data = load_aviris_swir_data(swir_base_path="AVIRIS_SWIR", tile_size=256, cache_file=cache_file, use_cache=use_cache)
    data = data.to(device)
    ...
    with torch.no_grad():
        ...
        # Save initial reconstruction visualizations
        save_hyperspectral_visualizations(
            sample_np, initial_recon_np, 
            recon_dir, "initial_recon"
        )
    ...
    for epoch in range(num_epochs):
        ...
        if current_condition_number < lowest_condition_number:
            ...
            # Generate reconstruction with this shape
            with torch.no_grad():
                lowest_cn_recon, _ = model(sample_tensor)
                lowest_cn_recon_np = lowest_cn_recon.detach().cpu().numpy()[0]
                
                # Save lowest condition number reconstruction visualizations
                save_hyperspectral_visualizations(
                    sample_np, lowest_cn_recon_np, 
                    recon_dir, "lowest_cn_recon"
                )
    ...
    with torch.no_grad():
        ...
        # Save final reconstruction visualizations
        save_hyperspectral_visualizations(
            sample_np, final_recon_np, 
            recon_dir, "final_recon"
        )
...
def train_decoder_only(model_path, shape_path, output_dir, noise_level, num_epochs=100, 
    ...
    shape_type = "initial" if "initial" in shape_path else "final" if "final" in shape_path else "lowest_cn"
    ...
    # Load AVIRIS_SWIR data - using 256x256 tile size
    data = load_aviris_swir_data(swir_base_path="AVIRIS_SWIR", tile_size=256, cache_file=cache_file, use_cache=use_cache)
    data = data.to(device)
    ...
    # Calculate final MSE and save reconstruction
    with torch.no_grad():
...
def plot_noise_comparison(noise_levels, initial_mse_values, lowest_cn_mse_values, final_mse_values, output_dir):
    """
    Plot the comparison of MSE values for initial, lowest condition number, and final shapes
    across different noise levels
    """
    plt.figure(figsize=(12, 8))
    
    # Plot lines for all three shape types
    ...
    plt.title("Comparison of Shape Performance Across Noise Levels")
...
def main():
    ...
    parser.add_argument("--cache", type=str, default="cache/aviris_tiles_swir_all_256.pt", help="Path to cache file for processed data")
    ...
    for noise_level in noise_levels:
        ...
        # Train decoder with lowest condition number shape
        print(f"\nTraining decoder with lowest condition number shape, SNR: {noise_level} dB")
