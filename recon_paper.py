...
class HyperspectralAutoencoder(nn.Module):
    def __init__(self, shape2spec_model_path, target_snr=0):
        super().__init__()
...
def train_and_visualize_autoencoder(model_path, output_dir, batch_size=10, num_epochs=500, 
                              learning_rate=0.001, freeze_shape=False, cache_file=None, use_cache=False):
    """
    ...
    freeze_shape: If True, freeze the shape parameters so only decoder is optimized
    ...
    # Set shape frozen if requested
    if freeze_shape:
        print("Freezing shape parameters - only decoder will be optimized")
        # Set requires_grad=False for shape parameters
    ...
    plot_shape_with_c4(initial_shape, "Initial Shape", initial_shape_path)
    ...
    plt.close()
    print(f"Initial filters saved to: {os.path.abspath(initial_filters_path)}")
    ...
    for i, band in enumerate(bands_to_plot):
        ...
        plt.imshow(sample_np[:, :, band], cmap='viridis')
        plt.title(f'Original Band {band}')
        plt.colorbar()
        
        plt.subplot(2, len(bands_to_plot), i+1+len(bands_to_plot))
        plt.imshow(initial_recon_np[:, :, band], cmap='viridis')
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % 10 == 0 or epoch == 0:
            ...
            if (epoch+1) % 50 == 0:
                plt.figure(figsize=(12, 8))
                for i in range(11):
                    plt.plot(current_filters.numpy()[i], label=f'Filter {i}' if i % 3 == 0 else None)
                plt.grid(True)
                plt.xlabel("Wavelength Index")
                plt.ylabel("Filter Value")
    ...
    plot_shape_with_c4(final_shape, "Final Shape", final_shape_path)
    ...
    plt.close()
    print(f"Final filters saved to: {os.path.abspath(final_filters_path)}")
    
    # Compare initial and final filters
    plt.figure(figsize=(15, 10))
    for i in range(11):
        plt.subplot(4, 3, i+1)
        plt.plot(initial_filters.numpy()[i], 'r--', label='Initial')
        plt.plot(final_filters.numpy()[i], 'b-', label='Final')
        plt.grid(True)
        plt.title(f'Filter {i}')
        if i == 0:
            plt.legend()
    plt.tight_layout()
    filter_comparison_path = f"{output_dir}/filter_comparison.png"
    plt.savefig(filter_comparison_path)
    plt.close()
    print(f"Filter comparison saved to: {os.path.abspath(filter_comparison_path)}")
    ...
    for i, band in enumerate(bands_to_plot):
        ...
        plt.subplot(2, len(bands_to_plot), i+1+len(bands_to_plot))
        plt.imshow(final_recon_np[:, :, band], cmap='viridis')
        plt.title(f'Final Recon Band {band}')
        plt.colorbar()
    
    plt.tight_layout()
    recon_path = f"{output_dir}/final_reconstruction_sample_{sample_idx}.png"
    plt.savefig(recon_path)
    plt.close()
    print(f"Final reconstruction saved to: {os.path.abspath(recon_path)}")
    
    # Visualize the encoded representation (11 bands)
    plt.figure(figsize=(15, 6))
    for i in range(min(11, encoded_np.shape[-1])):
        plt.subplot(2, 6, i+1)
        plt.imshow(encoded_np[:, :, i], cmap='viridis')
    ...
    plt.title("Filter Matrix Condition Number During Training")
    condition_plot_path = f"{output_dir}/condition_number.png"
    plt.savefig(condition_plot_path)
    plt.close()
    print(f"Condition number plot saved to: {os.path.abspath(condition_plot_path)}")
    ...
    plt.close()
    print(f"Training loss plot saved to: {os.path.abspath(loss_plot_path)}")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE")
    plt.title("MSE of Reconstruction and Input")
    mse_plot_path = f"{output_dir}/mse_values.png"
    plt.savefig(mse_plot_path)
    plt.close()
    print(f"MSE plot saved to: {os.path.abspath(mse_plot_path)}")
    ...
    np.save(f"{output_dir}/sample_{sample_idx}.npy", sample_np)
    np.save(f"{output_dir}/initial_recon_{sample_idx}.npy", initial_recon_np)
    np.save(f"{output_dir}/final_recon_{sample_idx}.npy", final_recon_np)
    np.save(f"{output_dir}/encoded_{sample_idx}.npy", encoded_np)
    ...
    with open(f"{output_dir}/training_params.txt", "w") as f:
        ...
        f.write(f"Freeze shape: {freeze_shape}\n")
    ...
    return model, losses, condition_numbers, mse_values, output_dir
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze-shape", action="store_true", help="Freeze shape parameters")
    ...
    model, losses, condition_numbers, mse_values, output_dir = train_and_visualize_autoencoder(
